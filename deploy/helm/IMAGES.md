...
| Image | Where | Licence | Why it is here |
|---|---|---|---|
| `ghcr.io/synveda/postgres:<appVersion>` | `postgres.bundled.image` | ours over PostgreSQL-licensed PostgreSQL | The existing pinned PostgreSQL 17.11 + pgvector image, reused for the operator-free persistent StatefulSet and bounded bootstrap. No new database distribution or operator. |
...
| `node:22-bookworm-slim@sha256:83f487e0a63425e5b4d146fb5e5be574bcbe1b7b843d3ebafdd95eaf7767a7e5` | gateway console stage | MIT | Builds the console bundle; its Node executable and licence also run short-lived release preparation. No Node server is added. |
