...
| [ADR-0104](adr-0104-indivisible-live-provider-effect-generation.md) | Indivisible live-provider effect generation | Accepted | Current (fixture-only pre-attempt plus conclusive-not-created attempted tail/recovery implemented; provider-process execution absent) | CPR-45 | Defines a sibling effect branch after the completed start decision, with a distinct effect witness, one-shot start-attempt fence, four-role causal identity and bounded recursive retirement. Process-free blueprint v2 binds exact planned runtime, role, endpoint and pinned conclusive-adapter commitments. The adapter can report only not-created; recovery exact-byte loads its module and validates the result but never calls its executor. Production has a deny-only, unregistered contract; ADR-0103's no-spawn branch remains immutable and terminal. No live effect execution, supported lifecycle or finalization capability is implemented or registered. |
