...
| [ADR-0104](adr-0104-indivisible-live-provider-effect-generation.md) | Indivisible live-provider effect generation | Accepted | Current (pure grammar implemented; state/effect execution absent) | CPR-45 | Defines a sibling effect branch after the completed start decision, with a distinct effect witness, one-shot start-attempt fence, four-role causal identity and bounded recursive retirement. Production has a deny-only, unregistered pure contract; a class-separated fixture validates the closed event grammar without consuming future persisted-state versions. ADR-0103's no-spawn branch remains immutable and terminal; no live effect execution, effect recovery, lifecycle or finalization capability is implemented or registered. |
