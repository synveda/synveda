...
| [ADR-0102](adr-0102-portable-reference-deployment.md) | Portable reference deployment contract | Accepted; amended by ADR-0104 | Current (implementation open) | CPR-45 | Compose is the canonical single-host reference; Keycloak replaces Rauthy, workers separate and optional Apalis remains an adapter. Its fixed-fake clean-Engine boundary has state-owned create/cleanup recovery. The live tuple grants effect-free plan, intent and start-decision state plus ADR-0103's terminal no-spawn reservation and ADR-0104's fixture-only pre-attempt and bounded conclusively-not-created attempted tail/recovery. Production remains deny-only and unregistered; no live process/provider effect, supported lifecycle or finalization capability follows. |
...
| [ADR-0104](adr-0104-indivisible-live-provider-effect-generation.md) | Indivisible live-provider effect generation | Accepted | Current (fixture-only pre-attempt, conclusive-not-created recovery and acknowledged missing-delivery terminal implemented; provider-process execution absent) | CPR-45 | Defines a sibling effect branch after the completed start decision, with a distinct effect witness, one-shot start-attempt fence, four-role causal identity and bounded recursive retirement. Process-free blueprint v3 binds exact planned runtime, role, endpoint and pinned adapter-v2 commitments. The adapter can report only not-created; the separate explicit same-UID missing-delivery path terminates as effect-possible without replay, receipt, close or marker retirement. Production has a deny-only, unregistered v1 contract; ADR-0103's no-spawn branch remains immutable and terminal. No live effect execution, supported lifecycle or finalization capability is implemented or registered. |
