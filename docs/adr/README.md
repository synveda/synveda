...
| [ADR-0104](adr-0104-indivisible-live-provider-effect-generation.md) | Indivisible live-provider effect generation | Accepted | Current (state implementation absent) | CPR-45 | Defines a sibling effect branch after the completed start decision, with a distinct effect witness, one-shot start-attempt fence, four-role causal identity and bounded recursive retirement. A deterministic read-only prerequisite projection records the unmet cut with every authority false and no production importer; it neither consumes the future state versions nor selects a branch. ADR-0103's no-spawn branch remains immutable and terminal; no live effect execution, effect recovery, lifecycle or finalization capability is implemented or registered. |
