...
| [ADR-0108](adr-0108-ci-gate-ownership-and-build-caching.md) | CI gate ownership and Kind build caching | Accepted; amended twice | Current (refactor hosted verification pending) | FND-1, OPS-8, OPS-12 | CI Result permits only declared skips; shared native CLI/Docker validation retains distinct acceptance. Tag publication requires exact-source full CI and verified candidate/asset inventories. |
