...
| [ADR-0104](adr-0104-indivisible-live-provider-effect-generation.md) | Indivisible live-provider effect generation | Proposed | Current (implementation absent) | CPR-45 | Proposes a sibling effect branch after the completed start decision, with a distinct effect witness, one-shot start-attempt fence, four-role causal identity and bounded recursive retirement. ADR-0103's no-spawn branch remains immutable and terminal; no live effect execution, effect recovery, lifecycle or finalization capability is implemented or registered. |
