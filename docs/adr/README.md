...
| [ADR-0104](adr-0104-indivisible-live-provider-effect-generation.md) | Indivisible live-provider effect generation | Accepted | Current (fixture-only pre-attempt state/recovery implemented; execution absent) | CPR-45 | Defines a sibling effect branch after the completed start decision, with a distinct effect witness, one-shot start-attempt fence, four-role causal identity and bounded recursive retirement. A closed process-free blueprint now binds exact planned runtime, role and endpoint commitments into that existing fence without a generation change. Production has a deny-only, unregistered contract; the class-separated seam never invokes a process. ADR-0103's no-spawn branch remains immutable and terminal; no live effect execution, supported lifecycle or finalization capability is implemented or registered. |
