...
CPR-32  Unified approvals across governed artifacts (XL)
  Filed 2026-08-25 by the autonomous continuation. Extend the existing
  VedaFlow proposal/change model rather than adding per-noun review queues.
  AC: every Knowledge, Skill, Tool server/binding, Configuration, policy
  relaxation and OKF-publication change exposes immutable typed artifact
  references on the common proposal; approval rules express required roles,
  distinct people, author self-review restrictions and optional separation of
  author, reviewers and effect executor, resolved from the effective inherited
  profile and curator requirements; approve/reject requests bind the exact
  proposal commit, rejection requires a reason, stale verdicts fail, and the
  proposer can cancel without impersonating a reviewer; auto-apply still opens
  and completes the same change when the matrix asks for no reviewer; Advanced
  Reviews uses generated API types to filter all artifact families, display
  requirements, immutable refs and lifecycle evidence, cast verdicts, cancel
  and execute approved effects; New Learnings remains the lightweight capture
  workflow and the distinct session-event secret quarantine remains a security
  control, while no duplicate artifact proposal/review handler or screen
...
CPR-38  Bounded graph-augmented retrieval (XL)
  Filed 2026-08-25 by the autonomous continuation. Expand only already-
  authorised current Knowledge anchors over the immutable KnowledgeRelation
  graph under explicit governed bounds, and retain re-authorised paths as
  ContextRun evidence rather than hidden score state. AC: at most two hops with
  bounded anchors, per-node fan-out, total expanded candidates, wall-clock and
  graph-token work; PDP before anchors/adjacency, after expansion and before
  trace rendering; six supporting edge types with contradiction as a zero-
  weight warning; separate anchor, edge, hop, freshness, current-state and final
  score components; full/redacted/hashes-only/disabled path retention; exact
  degradation with lexical/vector fallback on graph failure or budget; no
  stale/superseded/transitional current injection; generated Context Inspector,
  multi-hop quality fixture, audit, RLS, demo, `make ci` and `make db-test`
...
CPR-43  Final context-platform hard cut (XL)
  Filed 2026-08-26 by the autonomous continuation. Replace the development
  migration history with one epoch-3 context-platform baseline and delete the
  last replaced Record/runtime/client/frontend residue without a translator or
  compatibility layer. AC: one pure-DDL baseline; markerless and epoch-2
  refusal plus clean bootstrap/reset; complete forced RLS; regenerated SQLx,
  OpenAPI and generated clients; exact route/CLI rejection; a repository-wide
  classified-residue gate over every Prompt-33 search term; current docs;
  focused schema/security/client/demo/evaluation evidence, `make ci` and
