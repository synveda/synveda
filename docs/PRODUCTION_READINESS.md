...
| Area | Status | Severity | Evidence | Current test/monitor | Gap | Acceptance criteria | Suggested implementation slice | External dependency/owner decision |
|---|---|---|---|---|---|---|---|---|
| Release/Helm artifact parity and reproducibility | Not ready | P0 | The release publishes `ghcr.io/synveda/gateway` and a non-CNPG `ghcr.io/synveda/postgres` image (`.github/workflows/release.yml:261-281,338-349`). Helm instead defaults to `synveda/gateway` and the CNPG-compatible but unpublished `synveda/enterprise-postgres:17` (`deploy/helm/synveda/values.yaml:16-20,109-112`), and the release does not package/publish the chart. The source Dockerfiles build fail-closed. A historical kind run exercised the earlier chart, but `demos/ops-2-helm-install.sh` has not been run since the three-credential/install-job cutover. | `make chart-lint`; `make check-chart-images`; executable Helm/image-input checks; no current live source-image kind acceptance | Neither the current source installation nor pullable release parity is live-proven; a customer cannot pull the named product/CNPG image pair and chart from one release, and chart/application versions have no published atomic unit. | A tagged release publishes chart plus both pullable multi-arch images from the exact Dockerfiles it references; install in an empty registry-authenticated cluster completes login/context; every accepted chart enum boots. | Make the release workflow the one artifact authority, package/sign the chart, publish the CNPG image it actually references, and run the existing live acceptance from an empty authenticated registry. | Choose OCI chart/image registry and retention; release owner supplies publish credentials. |
