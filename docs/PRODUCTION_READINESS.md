...
published and pull-verified the chart and CNPG-compatible image set named by
Helm; the new existing-Secret KMS wiring has no production-shaped
...
| Area | Status | Severity | Evidence | Current test/monitor | Gap | Acceptance criteria | Suggested implementation slice | External dependency/owner decision |
|---|---|---|---|---|---|---|---|---|
| Dependency, npm, container and CI supply chain | Conditional | P1 | `cargo-deny` enforces sources, advisories and licence policy (`deny.toml:4-70`); CI also checks npm/corpus/container inventories (`.github/workflows/ci.yml:45-111`). Images/actions are tag-major or tag-version references rather than immutable digest/SHA pins (`.github/workflows/ci.yml:17-25`; `deploy/helm/synveda/values.yaml:16-20,109-112,169-176`). | `cargo deny check`; `make check-npm-licences`; `make check-chart-images` | Inventory is not vulnerability scanning or provenance. No container/npm advisory gate, image digest lock, CI-action SHA pinning or documented remediation SLA. | Every released dependency/image/action has an immutable identity, vulnerability result and approved exception with expiry; release blocks critical findings and emits the inventory used. | Add lockfile/container scanners and action/image pin checks to release CI; keep current licence gates. | Security owner defines severity SLA and exception authority. |
...
| Area | Status | Severity | Evidence | Current test/monitor | Gap | Acceptance criteria | Suggested implementation slice | External dependency/owner decision |
|---|---|---|---|---|---|---|---|---|
| Release/Helm artifact parity and reproducibility | Not ready | P0 | The source workflow packages the chart and is wired to build `ghcr.io/synveda/gateway`, the single-host PostgreSQL image and `ghcr.io/synveda/enterprise-postgres` at one validated release version. Helm defaults the product/CNPG pair to that `Chart.appVersion`; `make check-release-parity` packages twice and renders the exact pair without Docker or network access. Source Dockerfiles build fail-closed. A historical kind run exercised the earlier chart, but `demos/ops-2-helm-install.sh` has not been run since the three-credential/install-job cutover. | `make check-release-parity`; `make chart-lint`; `make check-chart-images`; executable Helm/image-input checks; no tagged workflow or current live source-image kind acceptance | No tagged candidate has published and pull-verified this atomic set. There are no captured OCI index/platform descriptors, versioned release environment manifest, signatures or current registry-backed install results. Static configuration and same-checkout chart-package stability are not registry or cross-checkout reproducibility evidence. | A tagged release publishes chart plus both pullable multi-arch images from the exact Dockerfiles it references; install in an empty registry-authenticated cluster completes login/context; every accepted chart enum boots. | Publish a candidate tag, capture exact source/deployment/chart and OCI index/platform digests in a standalone environment manifest, then authenticate, pull and run the existing live acceptance from an empty registry. | Choose OCI chart/image registry and retention; release owner supplies publish credentials. |
