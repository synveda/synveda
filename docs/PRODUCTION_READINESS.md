...
published and pull-verified the chart and five-image Compose/Helm artifact set;
the new existing-Secret KMS wiring has no production-shaped
restart/restore evidence or owned custody and rotation procedure; and there is
no backup/PITR and restore drill. P1 gaps also leave the gateway
...
| Area | Status | Severity | Evidence | Current test/monitor | Gap | Acceptance criteria | Suggested implementation slice | External dependency/owner decision |
|---|---|---|---|---|---|---|---|---|
| Release/Helm artifact parity and reproducibility | Not ready | P0 | The source workflow packages the chart and has one validated native amd64/arm64 plan for exactly five first-party images: gateway, single-host PostgreSQL, CloudNativePG PostgreSQL, optimized Keycloak and the reference proxy. Helm consumes the gateway/CNPG pair at that `Chart.appVersion`; `make check-release-parity` proves all five workflow tuples, packages twice and renders the Helm pair without Docker or network access. Deployment Dockerfile bases are tag-plus-full-digest gated. A historical kind run exercised the earlier chart, but `demos/ops-2-helm-install.sh` has not been run since the three-credential/install-job cutover. | `make check-release-parity`; `make chart-lint`; `make check-chart-images`; executable Helm/image-input checks; no tagged workflow or current live source-image kind acceptance | No image was built by the deterministic gate, and no tagged candidate has published and pull-verified this atomic set. There are no captured OCI index/platform descriptors, versioned release environment manifest, signatures or current registry-backed install results. Static configuration and same-checkout chart-package stability are not registry or cross-checkout reproducibility evidence. | A tagged release publishes the chart plus all five pullable multi-arch images from the exact Dockerfiles it references; an empty registry-authenticated reference host completes Keycloak login/context, and an empty cluster consumes its Helm pair and boots every accepted chart enum. | Publish a candidate tag, capture exact source/deployment/chart and OCI index/platform digests in a standalone environment manifest, then authenticate, pull and run the existing live acceptances from empty targets. | Choose OCI chart/image registry and retention; release owner supplies publish credentials. |
| Installer, upgrade, rollback and deployment parity | Not ready | P1 | Host install has checksum and executable smoke coverage, but upgrades reinstall/restart (`docs/INSTALL.md:818-832`). Helm is restart-shaped and the database hard cut has no translator. | Release-install demo; `make check-deploy`; chart lint | No supported rollback, N-1 compatibility test, zero-downtime upgrade, package manager or production Compose/Helm parity against the same published artifacts. | N-1 to N and failed-N rollback run in installed-host and Helm shapes with preserved data/key; versions/schema/contract are checked before traffic; previous artifact remains recoverable. | Add versioned release manifest, compatibility matrix and upgrade/rollback acceptance after fixing artifact parity and DR. | Owner sets support window and acceptable upgrade outage. |
| Platforms, version support and project licence | Not ready | P1 | Release builds macOS arm64 and Linux x86_64 and declares unsigned limits (`.github/workflows/release.yml:89-149,442-443`). There is no repository `LICENSE`, while OpenAPI metadata names `Proprietary` (`crates/synveda-gateway/src/openapi.rs:86`); README records the unresolved mismatch (`README.md:100-102`). | Native binary smoke in release workflow | Windows/Linux arm64 binaries and version windows are absent. A metadata label without complete owner-approved terms does not establish external distribution/use rights. | A support matrix names tested OS/arch/Postgres/Kubernetes versions and lifecycle; CI exercises each claimed target; owner-approved `LICENSE`/notices ship in every artifact and generated metadata agrees. | Add targets only after native CI/install evidence; reconcile the legal metadata and release contents through an owner decision. Do not invent a licence. | Legal/owner decision is required for the first-party licence and support policy. |
...
|---|---|---|---|---|---|---|
| PR-01 / release artifact parity | P0 | Make the tagged release workflow publish the exact chart and five-image Compose/Helm set; Helm consumes the same gateway and CNPG-compatible version. | No product-profile branch or chart redesign. | Pull all artifacts from an empty authenticated registry and complete login plus one context run; verify signatures/digests and existing chart contract checks. | Publish a candidate tag first; retain the previous atomic manifest and chart version for rollback. | OCI registry, release credentials, publisher identity and retention. |
