...
The paired public-API test can now opt in to exporting its four synthetic off/conservative rendered previews under ignored `target/`. A fixed answer rubric and prompt packager prepare the paired model input without calling a provider or mixing the answer key into prompts. A separate bounded CLI runner has a passing fake-client end-to-end test for explicit spend confirmation, no-tools calls, per-call caps, exact served-model identity, usage parsing and score output. No real model was called. This stages a later factual task-use comparison; it does not establish model quality or coding-task success. The exact export and run commands and evidence labels are in `evals/product/README.md`.
...
Another direct host index HTTPS probe still timed out on 2026-09-25. `cargo fetch --locked --offline` succeeds from this host's existing cache, but the canonical Dockerfile resolves its own dependencies inside the pinned build boundary; the host cache does not qualify that image or remove the source Compose blocker.
