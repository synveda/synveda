...
141 features parsed from docs/SYNVEDA_FEATURES.md — one file per
...
Demo evidence note (2026-08-24): entries below preserve what their original
feature scripts proved when delivered. CPR-13 subsequently replaced 49 stale
copies of pre-cut setup with current-platform narratives and focused current
acceptance seams. The checked-in script is now authoritative for what running
that demo proves; older prose remains historical evidence, not a support claim
for a deleted route or noun.
...
- [x] [HIER-1: Hierarchy store](HIER-1.md) — done 2026-07-18, **deleted whole by CPR-7 on 2026-08-20** (ADR-0074 decision 1): `hierarchy_nodes`, `hierarchy_closure`, `synveda_store::hierarchy`, `crates/synveda-store/tests/hierarchy.rs` and `demos/hier-1-hierarchy.sh` are gone, and no row was translated in either direction. What the feature proved — a closure-table tree with sub-millisecond ancestors/descendants at 10k nodes — is carried unchanged by `scopes` + `scope_closure` (CPR-3, ADR-0070), whose store suite is crates/synveda-store/tests/scopes.rs. Originally: AC test crates/synveda-store/tests/hierarchy.rs (10k nodes; ancestors/descendants medians 57µs/691µs over baseline).
...
- [x] [AUTHZ-3: Roles & role bindings](AUTHZ-3.md) — done 2026-07-19, **deleted whole by CPR-7 on 2026-08-20** (ADR-0074 decisions 1 and 6): `role_bindings`, `synveda_types::Role`, `/v1/roles/*`, `synveda role bind`, `crates/synveda-policy/tests/roles.rs`, `crates/synveda-gateway/tests/roles_routes.rs` and `demos/authz-3-roles.sh` are gone. Authority is a **grant** at a governed scope, inherited by its subtree (CPR-5, ADR-0072) and decided by the anchor model (CPR-6, ADR-0073) — crates/synveda-store/tests/access.rs, crates/synveda-gateway/tests/access_api.rs, crates/synveda-policy/tests/access.rs, demo: demos/cpr-5-access.sh. The admin-group bootstrap survives on the new noun (an `administrator` grant at the tenant root) and is still asserted by crates/synveda-gateway/tests/jit_provisioning.rs. Originally: AC tests crates/synveda-policy/tests/roles.rs (full role×action matrix per pack; escalation guard; subtree boundaries; privacy floor), crates/synveda-gateway/tests/roles_routes.rs (bindings govern the next request; delegation; uniform 404).
- [x] [HIER-2: Scope chain resolver](HIER-2.md) — done 2026-07-19, **deleted whole by CPR-7 on 2026-08-20** (ADR-0074 decision 2): the `ScopeChainCache`, its invalidation seam, its metrics, `crates/synveda-store/tests/scope_chain.rs`, `crates/synveda-gateway/tests/scope_chain_routes.rs` and `demos/hier-2-scope-chain.sh` went with the tree they cached. Chains now resolve per request through `scope_closure`, so "a move governs the very next request" is structural rather than cache-dependent — asserted by crates/synveda-gateway/tests/admin_scopes_api.rs and demos/cpr-7-scopes.sh. The latency discipline transfers to the anchor resolver (ADR-0073 decision 7 holds the deferred cache if measurement ever asks for one). Originally: AC tests crates/synveda-store/tests/scope_chain.rs (warm resolve median 800ns, p99 ≤1.5µs over 10k samples — 300× under the 0.5ms bound), crates/synveda-gateway/tests/scope_chain_routes.rs.
- [x] [HIER-3: Cedar entity sync](HIER-3.md) — done 2026-07-19, **re-cut onto governed scopes by CPR-7 on 2026-08-20**: the capability survives, the tree under it does not. AC tests: crates/synveda-gateway/tests/cedar_entity_sync.rs (a scope moved between org units governs the very next decision: the mover's authority leaves with it over HTTP; the parent's MemoryRead follows it at the composition seam), crates/synveda-policy/tests/entity_sync.rs (a warm fragment never survives a reshaped chain, both directions), demo: demos/cpr-7-scopes.sh (`demos/hier-3-cedar-entity-sync.sh` deleted with `/v1/hierarchy`).
...
- [x] [TEN-3: Dense-leg retrieval benchmark](TEN-3.md) — done 2026-08-10, ADR-0063 (**amended twice and corrected once during it**; the feature's own text was amended at the end, by decision 4, because the benchmark it asked for came back negative). AC demo: `demos/ten-3-dense-leg-sweep.sh` (10 arms × 3 runs, fresh database each, ~35 min). AC tests: `crates/synveda-store/tests/ann_bench.rs`. Rows: `evals/scores/ten3-dense-leg.json`, 20 of them, re-checked by `make check-ann-bench` in `make ci`. **The gate was fixed before the numbers existed and was not met**: partitioning ships only if, in the selective regime, it raises recall@10 at equal-or-better p95 or cuts p95 by ≥25% at equal-or-better recall@10 — and that regime measures recall **1.000 at 1.65ms p95** in all ten arms, on `records_tenant_scope_idx`, never touching the HNSW index. Nothing to raise, and nothing a `1/N` smaller index can cut. **The measurement also showed the gate was written in the wrong regime**, which is the finding TEN-7 carries: where hash partitioning *would* help is the broad regime under a custom plan (the HNSW index holds every tenant's vectors, ~7 of 8 candidates are discarded, recall 0.355 at ef_search 100) — but the product does not use that plan today, which is CTX-7. **What it found on the way, and this is the feature's real yield**: the benchmark disagreed with itself, measuring 0.341 recall at 5.91ms and 0.868 at 50.91ms for the same arm. Two things move the dense leg's plan and a harness recording one plan per run could see neither — statistics (a fresh corpus has none and autoanalyze arrives mid-loop; proven by deleting `pg_statistic` on a fixed corpus and watching HNSW → exact → HNSW after `ANALYZE`) and the custom-to-generic plan switch on the sixth execution of a prepared statement. Three of ADR-0063's four original findings were withdrawn as a result, including "iterative scanning is worth 2.6x recall and 8.6x latency", which is reproducible with iterative scanning *held constant*. **Two new axes returned clean negatives**: `max_scan_tuples` at twice the corpus moves nothing at any `ef_search` (0.355→0.365, 0.606→0.586, 0.773→0.775 — one the wrong way, all inside the spread), so the bound was never what stopped the scan; and `strict_order` is not separable from `relaxed_order` at equal latency. `ef_search` is monotonic once the plan is held still (0.355 → 0.606 → 0.773), so the "backwards row" that motivated the `max_scan_tuples` axis was the blend, entirely. **What it left standing**: the harness is the product's first retrieval performance harness and EVAL-6 inherits it rather than a blank page; the latency half is Docker Desktop's (one run of the first sweep took 1232s against ~66s for its siblings, which is why rows carry a spread and why the ADR's gate is relative); the corpus is generated, so `corpus_spec_digest` pins the vectors and not the graph — record ids are UUIDv7 and pgvector assigns HNSW levels at random; and **the publisher's guards were both written by failing**, first publishing n=1 rows and then publishing six arms of ten as though they were the sweep, which is why a sweep now declares its grid before the first arm runs.
_CPR-43 retirement note (2026-08-26): TEN-3 remains completed historical evidence in ADR-0063, but its Record-schema harness, score row and check-ann-bench gate were deleted. Current Knowledge/ContextRun retrieval and CPR-40 evaluation suites now own new claims; demos/ten-3-dense-leg-sweep.sh points to those tests and keeps no schema translation._
- [x] [TEN-4: Per-tenant encryption keys](TEN-4.md) — done 2026-08-11, ADR-0064 (**amended twice during implementation**: decision 12 named Cedar `Action` variants for two operator commands that reach no gateway route and pass no policy decision — they take *audit* actions instead, `tenant.key.provisioned`/`rotated`/`tenant.exported`/`tenant.secret.stored`/`cleared`, and the packs stay at `@16` with no golden re-recorded; and decision 5 turned out to have a consequence for decision 12 that neither noticed — see below). AC demo: `demos/ten-4-envelope-keys.sh` (**the AC's word is "unreadable without *that tenant's* key", so the demo is about the negatives**: the same archive bytes refused under another deployment's KEK, under no KEK, and — relabelled in its own cleartext header, which is what somebody holding the file would try — under another tenant's key; then the sealed columns checked from SQL rather than from the application that wrote them, a rotation that leaves the generation-1 archive still opening, and the chain verified with no credential material in any payload). AC tests: `crates/synveda-store/tests/keys.rs` (13), `crates/synveda-crypto` (39 unit), `crates/synveda-gateway/tests/console_session.rs` (12, now through sealed columns). New: `synveda-crypto` — a crate between `types` and the middle band, because store, identity and vedaflow all seal or open and the layering rule forbids them depending on each other. Migration 0038; `synveda kms keygen`, `synveda tenant key provision|rotate|status`, `synveda tenant export|export-open|export-describe`, `synveda directory set-credential|clear-credential`. **What the design turned on, and it was not the cryptography.** Seed §10 asks for all data encrypted at rest under per-tenant keys; applied to `records.content` and `record_embeddings` that deletes ADR-0024 outright, because there is no BM25 over ciphertext and no HNSW over ciphertext. §10 is a non-functional requirement rather than a §2 principle, so decision 7 scopes it out loud: per-tenant keys cover the artefacts that **leave** and the secrets we must **read back**, and the substrate's encryption at rest is the volume's. The consequence to keep in view is that **crypto-shredding is not erasure here** — destroying a tenant's key leaves its records exactly where they were, so TEN-5 still deletes rows. **The finding: `console_sessions` structurally cannot hold a tenant key.** The row is read *before* the tenant exists — reading it is one of the steps that establishes the tenant — so selecting a per-tenant key would need the derivation ADR-0056's schema refuses a `tenant_id` in order to prevent. The key plane gets a second scope rather than that table getting its tenant back; one extra scope is cheaper than one fewer isolation invariant, and ADR-0064 option 6 records the tempting wrong fix. **And that finding has a tail nobody saw until the code existed**: a failed open on that table cannot be a chain entry either, because the audit chain is per-tenant and there is no tenant yet — so it is `synveda_key_open_failures_total{scope,purpose}` and a log line there, and a chained event everywhere else. **The AC named an artefact that did not exist** — nothing exported a tenant, and `export = portable archive` is TEN-5's. Decision 8 reads that as a scope instruction and ships the smallest container that makes the claim testable: records and the audit chain, sealed under a fresh per-archive key wrapped by the tenant's, so handing somebody an export does not hand them the key to the tenant's live secrets. **Decision 9 deletes a named product limitation.** ADR-0060 decision 7 kept the outbound directory credential in deployment configuration because "the rows outlive the decision", and named this feature as its trigger; the credential now lives sealed in `tenant_secrets` with the whole connector configuration inside the envelope (sealing only the secret would let a credential and the host it is presented to disagree), the sweep resolves the table first and the deployment config second, and **a stored credential that fails to open skips the tenant rather than falling back** — falling back would re-point a customer at whatever directory the deployment happens to configure, on the day their key broke. One deployment can now pull two tenants from two directories. **What it left standing.** `Kms::Disabled` is the default and only `Local` is implemented — AWS, GCP and Vault are variants behind the same two operations and nothing has called one; the KEK lives in deployment configuration, so this defends a dumped table and a stolen archive and **not** an operator who can read the process's environment, which is the AUD-1 boundary and the whole of what is claimed. The key cache is per process with a 5-minute TTL, so a rotation on one replica is invisible to another until it expires — the `ScopeChainCache` staleness shape, and **OPS-7's** problem, which is survivable today only because the chart pins one gateway replica. Signing keys got custody's *shape* and not custody: ADR-0030 put key management here, but a signing key nobody verifies is a table row with no consumer, so `Signer::Unsigned` stays the default and FLOW turns it on. The export has **no re-import** and carries no assets and no destruction certificate — that is TEN-5, deliberately. `kek_ref` is a name an operator chooses, so two different KEKs sharing a name is a hazard the schema cannot catch (found by doing it: a suite left rows nobody could unwrap, and the message named `kms.data_key` rather than the KEK). And a deployment whose credentials live **only** in `tenant_secrets` needs `SYNVEDA_DIRECTORY_SYNC=on` to start the sweep at all, because `tenant_secrets` is under forced RLS and asking "does any tenant have one" is one transaction per tenant — a boot that is O(tenants) to answer a yes/no question. **What the tests found in the code**: `opening_key` selected a key by version and never checked the envelope's scope tag, so a tenant ring asked for a deployment-sealed payload read a row and spent a KMS unwrap before `open` refused it on the tag; and the deployment key is a per-database singleton, so two suites with different test KEKs leave each other rows nobody can unwrap — which is correct behaviour for a deployment and a coordination problem for a test suite.
- [x] [OPS-8: Release & distribution](OPS-8.md) — done 2026-08-11, ADR-0065, no migration. AC demo: `demos/ops-8-release-install.sh` (**the criterion, and the two things that make it mean something**: packaged release artefacts → `install.sh` → `init --demo` → a browser login → a governed recall, in **13s of the 600s budget**, run from a scratch directory that is not a checkout and with `cargo`, `rustc` and `rustup` shadowed by shims that exit 127 — so "no Rust toolchain" is a property of the run rather than a sentence in a README; then OPS-1's invariant re-asserted from the installed path, `chain valid (10 events)` with exactly one break-glass event, `/console/` returning 200, and a profile relabelled to another release refused by name). AC tests: `crates/synveda-cli/src/init.rs` (5 new unit tests: a bundle resolving every path `init` needs, a checkout resolving the way it always did, a version mismatch refused in both directions with a checkout exempt, a console bundle found only when `index.html` is there, and the per-architecture TEI table), `crates/synveda-cli/src/plugin.rs` (3: a marketplace accepted and a bare plugin directory named as *the* mistake rather than reported as not-found, an unknown client naming the ones it knows, and the installed id agreeing with the shipped marketplace manifest — a drift there installs something that is not there and Claude Code's error is about a missing plugin rather than about us). New: `.github/workflows/release.yml`, `scripts/install.sh`, `scripts/package-release.sh`, `scripts/package-plugin.sh`, `deploy/release/`, `adapters/claude-code/marketplace.json`, `adapters/claude-code/.mcp.json`, `synveda plugin install`, and the `release-install` CI job. **What it changed in the product, not just around it.** `init` grew a `Profile` — explicit > checkout > installed bundle, so a contributor who has also installed a release still gets the tree they are editing — and the shape of the failure it prevents is why the bundle carries a `version` the CLI compares against its own: a stale profile under a newer CLI presents as a service that will not start or a variable the gateway does not read, and both look like product bugs. **Three defects it found in code nobody had run this way.** The host-gateway path never set `SYNVEDA_CONSOLE_DIR`, so `/console/` had 404'd for every default install since CNSL-1 — quietly, because ADR-0056 decision 1 makes a missing bundle a 404 rather than a boot failure, which is the right call and is exactly what hid this. `synveda init --embedder tei` pulled the **amd64** TEI image on Apple Silicon: the Makefile has selected by `uname -m` since FND-2 and `init` never did, which nothing noticed while every install was a contributor's. And `connect()` refused with "DATABASE_URL must be set (dev default is in the Makefile)" — naming a file an installed operator does not have — so `synveda audit tail`, which INSTALL.md tells a new operator to run, failed on the very path this feature exists to create; it now defaults to the same URL `init` installs against. And three error messages suggested `curl … https://synveda.dev/install.sh | sh` — a domain with no DNS record, in the messages a person reads at the moment nothing is working; there is now one `INSTALL_COMMAND` constant naming a raw GitHub URL. **And three in the demo itself**, the last of them found by CI on the very first run — the demo assumed whoever ran it had already installed the workspace's node dependencies, which was true on a laptop where `node_modules` existed and false on a runner whose job primed only the console's filter, so the plugin build failed four minutes in. It installs what it needs now, so it runs on a fresh clone rather than only where something happened to prime the store. The other two, both the failure mode this repository keeps finding: its `EXIT` trap dropped the status, so the first run printed `demo FAILED: audit tail` and exited **0**; and it would have adopted a running dev stack's containers, because the dev compose and the released profile are both `name: synveda` — right for each of them and wrong for two at once, so it runs as its own compose project on prechecked ports. **And an upgrade left the previous release serving.** `install.sh` replaces `bin/synveda-gateway`, and the next `init` reported "already running with this configuration" and healthy while the old binary kept the port — measured on `v0.1.0` → `v0.1.1`, where the process had been up two hours and the binary under it was minutes old. ADR-0055 decision 10's "convergence compares configuration, not liveness" was right and insufficient: an upgrade changes neither, it changes the artefact. The fingerprint carries the gateway's length and modification time now. **That makes three, and the pattern is the finding rather than the bugs**: every check in this feature that sat one layer shallower than the claim it stood for passed against something broken — `GET /console/ → 200` against a console nobody could sign into, "the plugin files are in place" against a plugin that never loaded, "already running" against a release no longer installed. Each fix asserts the thing itself: sign in, ask the harness, compare the artefact. **And the admin console had never been signed in to.** A console session seals its tokens under the **deployment-scope** key (TEN-4, ADR-0064) and `init` never minted one, so every install booted `Kms::Disabled` and every sign-in failed with `not found: encryption key for deployment`, redirecting to `/console/?error=server_error` — **unusable on every fresh install since TEN-4**, because CNSL-1 built the console before TEN-4 gave it that dependency and nothing connected the two. The reason it survived *this* feature is the sharper half: OPS-8 fixed the console's 404 and asserted the fix as `GET /console/ → 200`, and a keyless deployment returns 200 too — the bundle is static and holds no data, so the assertion was one layer too shallow to see that nobody could get past the login. `init` now mints a KEK at `<state>/kms.key` (0600), defers to `SYNVEDA_KMS_KEY`, passes it on both start paths, and carries it into the convergence fingerprint as a hash so a keyless deployment restarts to pick one up; the demo now **signs in** — IdP round trip, `__Host-` cookie, a `/v1/whoami` that resolves the tenant — and a deployment with no key plane fails at the callback. **And the release shipped no way into the harness the product is for, which is what asking "how does it hook into Claude?" surfaced.** The prerequisite list was down to Docker and a tester still could not put governed memory in Claude Code: the plugin lives in `adapters/claude-code`, its `dist/` is gitignored, and nothing in the release built or carried it — the only route was to clone the repository and run `npx tsc`, which is the exact barrier this feature removes, surviving where it mattered most. So there is a fifth asset and a new verb, `synveda plugin install` (ADR-0065 amendment 2), and building it found **three more defects, two of them in code ADPT-1 shipped a year ago**. `plugin.json` declared `"hooks": "./hooks/hooks.json"` — the file is read automatically, so declaring it too is a duplicate-load error and **the plugin has never loaded in Claude Code**, reporting `✘ failed to load` while every other part of the install looked healthy. It also declared `mcpServers` inline, which is **silently ignored** — the server map belongs in `.mcp.json` at the plugin root — so the plugin advertised an MCP server it never registered (`MCP servers (0)`). And `~/.claude/plugins/synveda/`, the path the adapter README told people to use and `demos/adpt-1-claude-code.sh` still copies into, **is not a location Claude Code reads**: plugins come from a *marketplace* carrying `.claude-plugin/marketplace.json`, installed into a cache it owns. **Why nothing caught any of it**: ADPT-1's demo is its own harness — it reads `hooks/hooks.json`, substitutes `${CLAUDE_PLUGIN_ROOT}` and invokes node itself, which tests the hook contract exactly as its AC asks and can say nothing about whether the real harness ever loads the plugin. ADR-0027 is amended; `scripts/package-plugin.sh` refuses to build a bundle that reintroduces either manifest key; and the demo now asserts the vendor's own view — `✔ enabled`, four hooks, one MCP server — rather than the presence of files. The verb **drives `claude plugin`** rather than writing Claude Code's three cross-referencing JSON files: ADR-0057 decision 10 justified writing another application's config by the absence of a CLI, and Claude Code has one, so the justification does not transfer (seed §2 principle 6, from the other end). **What it left standing**: a live Claude Code *session* still has not injected or observed — the plugin is proven to load, not to work end to end, which is ADR-0057 decision 11's Cursor gap in a different place; **unsigned and un-notarized** binaries, which is a purchase and an identity rather than an engineering decision, so the installer strips the quarantine attribute and says so; **no Windows**, **no upgrade path** (reinstalling is how you upgrade) and **no package manager**; **macOS arm64 and Linux x86_64 only**, with the installer refusing anything else by name and naming the `cargo build` that still works, because an installer that guesses fails later and inside the product; and the release workflow is now **the one thing that can break without a red build to say so** — nothing in `make ci` runs it, and `workflow_dispatch` building without publishing is the whole of the answer. **That dry-run mode paid for itself twice before a tag existed** (ADR-0065 amendment 3). `bundles` built the console tarball perfectly and died *listing* it — `tar -tzf | head -n 3` closes the pipe, tar takes EPIPE, and `pipefail` promotes that to a failed job — worth keeping because it is **non-deterministic**: the same pattern exits 0 locally against a *larger* listing, since tar finishes writing into the pipe buffer before head closes. And **multi-arch images under QEMU are unusable rather than slow**: one job building amd64+arm64 through `setup-qemu-action` hit its 90-minute timeout with the *gateway image alone* unfinished, so Postgres never started and `publish` never ran. Images now build one architecture per **native** runner (`ubuntu-24.04-arm`, free for public repositories) with a manifest join at publish time; dropping arm64 was the alternative and is worse than it sounds, because `ghcr.io/synveda/postgres` is pulled by *every* install rather than only the `--issuer` path, so an amd64-only image puts every Apple Silicon tester under emulation in the one feature whose subject is that installing should be easy. **The residue a dry run cannot reach**: the manifest join and `gh release create` are publish-only, so the first real tag is the first thing to exercise them. **And then the default install path turned out to be the one nothing had run** (ADR-0065 amendment 6), found while restoring a machine to a released `v0.1.1` in order to test an upgrade. `install.sh` puts the CLI in `$SYNVEDA_BIN`, default `/usr/local/bin`, which is root-owned on macOS; that branch tested `command -v sudo` and then ran `sudo cp` unguarded, so **a sudo that ran and *refused* killed the script under `set -e`** — after the gateway, console, profile and plugin were already on disk, leaving a **complete install with no CLI on PATH** and sudo's own error as the only explanation. The fallback to `$SYNVEDA_HOME/bin` existed and was unreachable: it was guarded on sudo being *absent*, never on it failing, and failing is the ordinary case — a managed laptop where the user is not an admin, a pipe with no terminal to prompt on (CI, a Dockerfile, `ssh host 'curl … | sh'`), or somebody who declines. Two smaller faults sat in the same block: `sudo cp` wrote **over** the target where `install_file` deliberately does temp → `chmod` → rename, and overwriting a live Mach-O leaves a signature that no longer matches its contents so macOS kills the next run with `Killed: 9` — an *upgrade's* failure, and therefore the one least likely to be met before a user meets it; and nothing checked whether the directory the CLI landed in was on `PATH`, while the next-steps block printed four `synveda …` commands regardless. **The reason it survived is this feature's own pattern for the fourth time**: the demo hands `SYNVEDA_BIN` a directory it creates itself, so it only ever took the writable branch, and the default — the path every real `curl … | sh` takes on a Mac — was never run by anything. Sudo failure is recoverable now, the sudo path renames rather than overwrites, the PATH hint prints when it applies, and the demo installs a `sudo` shim that refuses and asserts the install still completes, that the CLI is in the fallback and *not* in the unwritable directory, and that the hint was printed. The default stays `/usr/local/bin` and sudo is still tried: an install that can use the conventional location should, it just must not *require* a privilege it never needed. **And then `init` reported a healthy gateway that had never started** (ADR-0065 amendment 7), found in the next minute of the same session. Decision 5 lets a checkout and an installed release coexist and `Profile` makes the checkout win — but both use port 8120, and `init` could not tell whose gateway was on it. Run from a checkout with the installed release up, it spawned a gateway that died in milliseconds with `AddrInUse`, and `wait_for_health` then asked `127.0.0.1:8120/healthz`, **which the installed gateway answered** — so it printed `pid 51544`, `healthy` and `initialised in 6s`, wrote that pid to `gateway.pid`, and exited **0** for a process that never bound anything. The health probe asks *is something answering here* and every claim built on it reads as *is the thing I just started answering here*, which are the same sentence only when nothing else can be on the port — the assumption decision 5 removed. **The first fix did not work, and that is the part worth keeping**: watching the pid we spawned looks sufficient and loses the race, because the gateway connects to Postgres, reads its key and starts five workers before it binds, so a child doomed to `AddrInUse` is still alive for the first second while the stranger answers instantly — measured, not reasoned. What closes it is asking for the port: `init` binds `127.0.0.1:8120` and releases it before spawning, and refuses when it cannot, naming the likely cause and why starting anyway would be worse. The liveness check stays for the *other* startup failures — a bad migration, an unreadable key — and carries the tail of the gateway's own log into the error so `Address already in use` reaches a person instead of staying in a file. The restart path gained the other half: it killed the old gateway and slept a fixed 500ms, a guess in both directions and now a hazard, since a gateway slow to shut down would be reported as a stranger; it waits for the port, up to ten seconds. Three unit tests cover it — a held port refused, a free one accepted, a dead pid reporting `Address already in use` out of its log — and the first version of the port test was itself **flaky**, asserting that a just-released ephemeral port was free again while every other test in the binary was free to take it; it asserts on port 0 now. **And v0.1.2 — the release that fixed an upgrade — shipped with a second upgrade still broken** (ADR-0065 amendment 8), found by checking the plugin on the machine that had just taken it. `install.sh` replaces `$SYNVEDA_HOME/plugin`, but Claude Code copies a plugin into a versioned cache **it** owns at install time, so replacing the marketplace leaves the *running* plugin wherever the release that installed it left it: the bundle on disk said `0.1.2` and `claude plugin list` said **`0.1.0`**, two releases behind, `✔ enabled`. `synveda plugin install` could not fix it in two independent ways — it asked whether the plugin was **installed** rather than at which version and reported "already installed — leaving it alone" (identity where the question is the artefact, which is amendment 5 one release later), and `--force`, the escape hatch that very message advertised, **had never worked**: it appended `--force` to `claude plugin install`, which takes no such flag, so it exited 1 with `error: unknown option '--force'`. Claude Code has no update verb and `install` on an installed plugin reports success while changing nothing, which is how it stayed invisible; replacing one is `marketplace update` (so the manifest is re-read, which `add` on a known marketplace does not do), then `uninstall`, then `install`. The command compares versions and does that when they differ, leaves a match alone, and `--force` replaces regardless — verified on this machine through all three paths, ending `0.1.2 ✔ enabled` with four hooks and one MCP server. **The finding under the finding**: the fix was aimed at the bug rather than at its shape, so the same fault one directory away went unlooked-for — "compare the artefact, not its identity" applies wherever this product caches somebody else's copy of what it ships, and the plugin is the one place it does. INSTALL.md's upgrade section now says to run it.
- [ ] [OPS-9: Beta demo profile](OPS-9.md) — filed 2026-08-13, directly behind OPS-8. OPS-8 asked whether somebody else can install this product and answered it; the question nobody had asked is what that somebody **sees** once they have. They see an empty product. `synveda init --demo` seeds four people into the bundled IdP and then *prints* the commands that would build ACME's scopes — nothing creates them — so a tester who follows INSTALL.md lands, correctly authenticated, in an organisation with no scopes, no memory, no proposals and an empty console, and the governance machinery that is this product's whole differentiator is invisible until they hand-run a dozen verbs nobody handed them. **The printing is not the defect**, which is what makes this a design rather than a fix: ADR-0055 decision 1 is explicit that these are governed creates needing the operator's own bearer, decision 2 is the invariant it protects, and `demos/ops-1-smb-profile.sh` asserts that invariant — 0 scopes, 0 identities, 0 role bindings, 0 records the moment the installer finishes, exactly one break-glass event in the chain. An installer that seeded an org would stand ACME's hierarchy under a break-glass actor, which is the opposite of what the product sells. So the seeding **moves** rather than being abolished: `synveda demo seed` runs after login, under the operator's own bearer, through the same routes the CLI drives, which leaves OPS-1's invariant exactly where it was because that invariant is about what an *installer* may do and this is not one. It ships in the binary rather than as a `demos/` script because the audience is exactly the person who ran `curl … install.sh | sh` and has no checkout — and because the 60 scripts in `demos/` could not serve this even if they were reachable, each being a per-feature acceptance proof that seeds its own scratch state and tears it down, none of them a tour. What it seeds is a *living* org: packs in deliberate contrast, a corpus through the real observe → extract → embed path, a published memory with channel history, two proposals left **pending** so the inbox holds something, and an active lapse so "strict by default, relaxable by design" has an instance to look at. The chain is not seeded; it falls out, and the AC reads it back. **Two findings came out of reading the approval matrices before writing any of it, and both changed the design.** The first is that the demo needs no second author, because the matrices already produce every state from one principal purely by where content lands: under `standard` a memory at a **team** matches no rule and the floor covers only `Restricted` and `Skill`, so it publishes with **zero** approvers and the channel gets history; the same publication at a `regulated-strict` **department** takes curator + steward, two distinct, so it sits pending and the operator cannot approve their own; and a **skill** takes a SecurityReviewer and two distinct under *every* pack, from the invariant floor. So the pack contrast is not decoration in the explorer — it is the mechanism, and the tester meets dual control by trying to approve and being refused rather than by reading that it exists. The plan had been service identities (AUTH-3) authoring what the operator could not review alone; that was solving a problem the product does not have. The second finding is sharper and outlives the demo: **a named human cannot be pre-authorised on this path at all.** A role binding is keyed on the OIDC `sub`, which the IdP assigns and the product learns when the person first presents a token, and there is no `/v1/identities` route to resolve an email to a subject — `identities` is reachable over HTTP only for *service* identities. Pre-provisioning a named person is SCIM's job (AUTH-4, which creates the identity ahead of login for the first login to adopt) and the bundled IdP is not a SCIM source. `role_bindings::bind` would have accepted the binding anyway — it is an upsert with no foreign key to `identities` — which is exactly why it is written down: the row would grant authority to a string matching nobody and the demo would look correct while being wrong. What the demo leans on instead is JIT placement by convention group, which needs the scopes to exist and nothing else. Two things it explicitly does not do: it adds no capability — every surface it seeds exists and every call it makes is one the CLI or a harness already makes, and what it removes is the requirement that the person watching already knows the verbs — and it does not close **ADPT-8**, so a headless Claude Code session still injects and never observes, which `docs/BETA.md` says rather than routes around. That file is the tour and the standing limits together, on the reasoning that a demo is driven by whoever built it and a beta is driven by somebody who did not, so the limits stop being things we remember to mention and become a file.
- [ ] [OPS-10: Uninstall & cleanup](OPS-10.md) — filed 2026-08-13, the mirror of OPS-8. That feature made the product installable by a stranger and gave them no way to remove it: there is no `uninstall.sh`, no `mcp uninstall`, no `plugin uninstall`, and `grep -rn uninstall` over the CLI and the scripts matches only a comment in `plugin.rs` describing how *Claude Code* replaces a plugin. A beta asks somebody to run this on a machine they own, so "how do I get rid of it" is a question to answer before it is asked — and prose does not answer it, because the footprint has **three tiers with three owners** and only one is safe to delete without thinking. **Ours**: `install.sh` keeps to the footprint its own header documents, and `init` adds `$SYNVEDA_HOME/data` — pidfile, log, rendered environment, and `kms.key`. **The deployment**: containers and four named volumes, where stopping is safe and removing the volumes is the **only way to remove a tenant's memory**, because TEN-5 means the tenant row cannot be deleted — so data survives by default, `--purge` takes it, and the message says that memory is what it took. **Somebody else's**: a `synveda` key inside Claude Desktop's, Cursor's, Zed's, VS Code's, Windsurf's or Continue's config, and a plugin in a cache Claude Code owns. `install.sh` deliberately writes none of that and says so; uninstall must be equally surgical, which makes removal the exact mirror of `mcp install` — take out the one key we own, write every other byte back as found — and that is the same CST splice, so it belongs in the CLI beside the verb that wrote it rather than in a shell script reimplementing it. The `plugin uninstall` assertion reads `claude plugin list` rather than the filesystem, on ADR-0065 amendment 2's finding that installing and loading are different events, and so are removing and unloading. It deletes no tenant, because nothing can.
- [ ] [TEN-5: Tenant lifecycle](TEN-5.md) — sharpened 2026-08-12 by OPS-8, which tried to remove one test tenant and could not. **`tenants` is referenced by 32 foreign keys, every one `ON DELETE NO ACTION`**, so `delete from tenants where …` succeeds only for a tenant that holds nothing — never one somebody has logged into, since the login alone writes `hierarchy_nodes`, `identities` and `role_bindings`. `demos/ops-1-smb-profile.sh` printed exactly that `delete` as its teardown instruction from OPS-1 until this was found, immediately after building a hierarchy and observing a turn; it cannot have worked once, and now names `synveda tenant export` (TEN-4) plus `docker compose down -v` instead. The finding is a **design input rather than only a defect**: `NO ACTION` everywhere is right for a product whose claim is that memory does not disappear quietly, so erasure has to be a deliberate ordered traversal — and the order is not free, because destroying `tenant_keys`/`tenant_secrets` before the rows they seal leaves ciphertext nobody can read while the row count still says it exists. The AC gains a bound the old one lacked: zero rows across all 32 tables, since a test that deletes a never-logged-into tenant would pass against today's schema and prove nothing.
- [ ] [TEN-6: Cross-tenant isolation test harness](TEN-6.md)
- [ ] [AUD-3: WORM export](AUD-3.md)
- [ ] [AUD-4: SIEM streaming](AUD-4.md)
- [x] [GRPH-3: Graph-augmented recall](GRPH-3.md) — delivered by CPR-38 on 2026-08-25 from `7951d77`, ADR-0097. The Record-era graph and feature flag are deleted; governed Configuration instead switches bounded anchor-first expansion over immutable Knowledge relations. The public acceptance proves the same query misses its second-hop answer with expansion disabled and selects it with a complete authorised path when enabled, while a private endpoint contributes no address, path, count or reason. `demos/cpr-38-bounded-graph.sh`, `make ci` and fresh-database `make db-test` pass; the resulting commit is recorded by CPR-39.
- [ ] [EVAL-6: Load & latency suite](EVAL-6.md)
- [ ] [CTX-7: Dense-leg plan stability](CTX-7.md) — filed 2026-08-10 by TEN-3 (ADR-0063), whose benchmark found it by disagreeing with itself: the same arm, same corpus, measured recall 0.341 at p50 5.91ms on one run and 0.868 at 50.91ms on another, and the variable was not the arm. The dense leg is a prepared statement on a long-lived pool, so PostgreSQL plans it against real parameters for five executions per connection and then substitutes a **generic** plan built without them — and the generic plan does not use `record_embeddings_hnsw_1024` at all, scanning the tenant's whole allowed slice through `records_tenant_scope_idx` and sorting exactly. Holding statistics fixed, the same statement with the same arguments takes HNSW on execution 1 and the exact scan on execution 6; `plan_cache_mode = force_custom_plan` keeps HNSW. Measured on TEN-3's final sweep (64k records / 8 tenants / dim 1024, n=3, idle machine): the generic plan gives recall **1.000** at p50 51.08ms, what ships (`auto`) gives **0.856** at 50.03ms, and the custom plan at the shipped `ef_search` 100 gives **0.355** at 6.14ms — with 400 and 1000 filling the curve at 0.606/16.29ms and 0.773/29.48ms. **This is a trade rather than a defect with an obvious fix, which is a correction to how it was first filed**: forcing custom plans at the shipped tuning would trade recall 1.000 for 0.355 to gain 8x, the worst point on the curve, and buying the recall back with `ef_search` spends the latency again — 0.773 at 29.48ms is the best HNSW arm and still short of exact at only 1.7x its speed. **So the defect is that nothing chooses.** The read path runs both plans, and which one is a function of how many times a pooled connection has executed the statement; two identical queries return different recall depending on the age of the connection they land on, and 0.856 is not a designed behaviour but the arithmetic of five custom executions out of twelve, moving with pool size and request rate. That is worth fixing even if the ruling is "keep the exact plan", because a benchmark, an SLO and a customer's reproduction all mean something different once the plan is stable. Two things keep the exact plan from being the automatic answer: it is O(tenant), so its 51ms at 8k records/tenant cannot be extrapolated to 1M from one corpus size; and the selective regime is already exact under both plans (125 rows, ~1.33ms, recall 1.000), so the ruling changes nothing for the regime the PDP's slice makes narrow. The selective regime is unaffected and exact under both plans. Phase 3, beside EVAL-6, because this is a performance claim the product has **already published** rather than one it has yet to make: CTX-1's AC is "p99 <80ms at 1M records/tenant", its test runs 200 queries through a pool at dim 16 where an exact scan is cheap, and nothing has established which plan that number was measured against. Not a governance hole — no answer is wrong, and the PDP's slice is applied identically by both plans.
...
- [x] [ADPT-8: Observation that survives a session that does not wait](ADPT-8.md) — done 2026-08-24 by CPR-14, ADR-0027 amendment 3. The original installed-client finding remains the reason for the feature: Claude Code 2.1.228 headless runs injected and never observed because every write hook was async. The 2.1.241 live gate closed the ambiguity: an async Stop did not complete before successful `claude -p` teardown, leaving an active Synveda session with zero events. The delivered seam makes Stop and PreCompact synchronous **only through the atomic local spool write** and returns before credential resolution or network; SessionEnd, the next SessionStart and explicit flush own delivery. The adapter suite asserts Stop makes no event request and leaves pending entries at `delivery_attempts = 0`. The installed authenticated client then reported plugin 0.2.0 enabled with four hooks and one MCP server, composed one context run, persisted four authentic user/tool/assistant events, ended the same session with reason `other`, and left a verifying audit chain. Stop's local boundary measured 8ms. The honest remaining tail is unchanged: a host killed before any lifecycle hook receives the in-flight turn can lose it; nothing which reached the spool is lost.
...
- [x] [CNSL-4: Knowledge browser](CNSL-4.md) — done 2026-08-24, subsumed by CPR-17/ADR-0082. The original mutable-record/channel design is deleted; its product objective is the generated-contract Knowledge Browser at `/console/knowledge`, with scope filters, immutable history, independently authorised provenance and typed VedaFlow lifecycle changes. AC tests: `crates/synveda-gateway/tests/knowledge_lifecycle.rs`, `console/src/knowledge.test.mts`; demo: `demos/cpr-17-knowledge-browser.sh`. No direct mutation or second hand-written API contract exists.
...
- [ ] [TEN-7: LIST partitioning per tenant](TEN-7.md) — filed 2026-08-10 by TEN-3 (ADR-0063 decision 4), which measured the partitioning its own text asked for and declined it. `LIST` rather than `HASH` because a hash partition holds an arbitrary set of tenants, so it can be neither dropped for one nor pinned for one, and every feature that wants partitioning wants exactly one of those — TEN-5's disposal, OPS-3's residency, and ADR-0009's own reversal trigger. Two independent triggers reopen it. The **boundary** one: TEN-5 or OPS-3 needs a per-tenant drop or pin, which is not an ANN question at all. The **ANN** one, conditional and numbered: CTX-7 must first rule that the dense leg runs the custom plan — until then post-filtering by tenant costs nothing because the HNSW index is not being used — after which this reopens if broad-regime recall@10 stays below **0.90 at p95 ≤ 30ms** over ≥8 tenants on the best tuning TEN-3's harness can find. On current evidence that trigger *would* fire (the measured ceiling is 0.773 at 30.98ms p95 at `ef_search` 1000), which is exactly why it is written down rather than left to be rediscovered. What it costs is unchanged: `records_pk` becomes `(id, tenant_id)` and **the schema stops asserting that a record id means one record**, the bitemporal triple and both archive triggers come along, and it is an operator-run offline repartition with a stated outage rather than a migration. Phase 4 because both triggers are contingent on other features, and because partition-per-tenant has a planning cost that grows with tenant count — the first thing it has to measure, and the reason ADR-0063 refused `LIST` for its own purposes.
...
_Programme goal: one product an individual, a team and an enterprise all run,
differing only in the policy profile assigned to their scopes. Thirty-three
ordered prompts; the decisions are locked in ADR-0068 and the running record
— inventory, deletion map, prompt order and the MVP checkpoint after Prompt
20 — is [docs/implementation/synveda-context-platform.md](../implementation/synveda-context-platform.md)._

- [x] [CPR-1: Implementation baseline & locked decisions](CPR-1.md) — done 2026-08-17, ADR-0068, no migration and no runtime change. What it produced is a **record rather than a behaviour**, so its acceptance is what the record says and whether it is true: `docs/implementation/synveda-context-platform.md` at base commit `92ffa89` — 54 `/v1` route paths beside 7 SCIM and 8 ops/auth ones, 24 CLI top-level commands split into gateway clients and store-level break-glass, a console with **no router and no navigation** (one page, four conditional sections), 38 migrations creating 49 tables and 2 views of which **46 carry forced RLS** and the three that do not each say why in their own migration header, 3 Cedar entity types and **34 actions** with the two places the organisational rank vocabulary reaches the PDP named (`Scope.kind`, `Principal.department`), and the observe/inject/recall paths traced end to end. Two findings the inventory produced rather than transcribed. **There is no session**: `session_id` is a `text` column on `observe_events`, an `Option<String>` on two request bodies and an audit correlation field, so the thing an agent actually does has no aggregate and cannot be governed, retained or asked about — which is why it is one of the eight locked decisions rather than a schema detail. And the **adapter verification levels are lower than the feature list implies**, three of them: a headless Claude Code run injects and never observes (ADPT-8, measured), no real Cursor frame has ever been replayed though `install` targets it, and the SCIM corpus is transcribed from Entra's and Okta's published tables rather than captured from a live tenant. ADR-0068 locks eight decisions against the cheaper option each refuses — notably the migrator, which is refused because mapping five ranks onto an unranked tree and re-deciding the derived/published boundary per row would put both in a script nobody reviews as carefully as a policy, bought with nothing pre-1.0. Test state recorded rather than asserted: `make ci` **PASS** and `make db-test` **PASS** at the base commit, **no pre-existing failures**, with the `--ignored` suites named as not run rather than left to be assumed.
- [x] [CPR-2: Fresh schema epoch, startup guard & local reset](CPR-2.md) — done 2026-08-17, ADR-0069, AC test: crates/synveda-store/tests/epoch.rs (10) + crates/synveda-gateway/tests/observability.rs (`readyz_refuses_a_database_that_is_not_at_this_schema_epoch`), demo: demos/cpr-2-schema-epoch.sh. Prompt 2, and the first one with a runtime. ADR-0068 decision 3 said a pre-cut database is "rejected at startup with a reset instruction"; this is the mechanism, and it was written against three holes in that sentence rather than one. **Nothing in the database said which model its rows were in** — 38 migrations, no marker, so `MIGRATOR.run` would bring an old database forward, which is the silent acceptance the decision forbids and was reachable by running the command INSTALL.md already told operators to run. **`_sqlx_migrations` is not that marker and using it would have been worse than none**: it answers "which of *this binary's* migrations have run", a question about a chain Prompt 33 squashes, and it moves on every ordinary release — a guard edited routinely is a guard nobody reads. **And the instruction named nothing**: `uninstall.sh --purge` destroys the Docker volumes, which takes Temporal's two databases with it (they share `pg-data`) and removes the installation rather than resetting the database. So: `schema_metadata` — one row, epoch + migration head + creation time + the release that created it, `created_by_version` written once and never rewritten because "which release minted this database" is a fact about the past; a **preflight before the migrator** that refuses any database with a schema and no marker, so a refusal leaves the rows byte for byte as it found them; the **boot guard** the gateway exits non-zero past; the **same check on `/readyz`**, which is not redundant — the gateway may start without a database (ADR-0007), so a database arriving afterwards would otherwise be behind a check that had already run; and `synveda reset --database --force`, which drops and recreates the *database* rather than the volume or the installation, preserving the KEK, the profile, stored logins and Temporal's data. Three judgements worth reading: an **outage is not a verdict** (unreachable is the one variant the gateway boots through — reporting a network problem as a wrong epoch would send somebody to destroy a database over it); a **newer** epoch refuses *differently*, naming the reset command only to say not to run it, because that database holds data this build cannot read; and `--force` **does not** destroy a database on another host, since it says "yes, destroy it" and not "and I checked which server I am pointed at". "No old-to-new data migrator exists" is asserted rather than asserted-to: a reset carries **zero** rows across, a refused migration writes nothing, the epoch migration is checked statement by statement to be pure DDL, and any `.down.sql` fails the test. The migration chain is deliberately **not** squashed — that is Prompt 33's, and doing it here would leave the guard with no pre-cut database to be tested against.
- [x] [CPR-3: Generic governed scope substrate](CPR-3.md) — done 2026-08-17, ADR-0070, AC tests: crates/synveda-store/tests/scopes.rs (20) + the scope block of crates/synveda-store/tests/rls.rs (4). Prompt 3, and the first one that builds a piece of the new model. What it removes is a **rank**: `hierarchy_nodes` says the root is an `org` in a row-local CHECK and that a child's kind must strictly outrank its parent's, so one person cannot hold a record here without first declaring a company containing a team. `scopes` + `scope_closure` replace it with a named node, a parent and a subtree — five **shapes** (`tenant`, `org_unit`, `workspace`, `project`, `principal`) whose only job is deciding which shapes may be their parent, so `org_unit` nests inside itself to whatever depth a deployment needs and one person's entire tree is a tenant scope and a principal. No `rank()`, nothing anywhere comparing two kinds for order. The judgement worth reading is **where each rule lives** (ADR-0070 decision 2): every structural rule that can be a database fact is one, so the placement rule rides a composite foreign key over a denormalised `parent_kind` rather than a store-side check the next caller can go around; a cycle is refused by a CHECK on the closure's own self-row — the row a move's relink would have to write — and not only by the descendant test the service makes first; and cross-tenant immobility holds for the **owner** role too, which is what migrations, break-glass psql and a restore run as and what forced RLS does not bind. Two things it deliberately does not do: it exposes **no API** (no route, no CLI, no console, no adapter, and no PDP call inside the store — the governed entry points attach at the boundary Prompts 5+ add), and it **synchronises nothing** with the old hierarchy, which is untouched and deleted whole by Prompt 6. Three findings from building it. `path` and `depth` are gone as columns — derived from the closure, so they cannot be stale and a move stops rewriting every descendant's copy. `status` carries a two-value vocabulary of which only one value is ever written, because the domain model states it and no service transitions it yet — recorded rather than quietly dropped. And the concurrency test that matters (a create landing inside a moving subtree) **still passes with `move_scope`'s subtree lock removed**, because the relink's foreign keys already take a share lock on every subtree member: the lock stays because the ordering should be stated rather than inherited from a foreign key, and the test says so rather than claiming a proof it does not have.
- [x] [CPR-4: Workspaces, projects & canonical repository identity](CPR-4.md) — done 2026-08-17, ADR-0071, migration 0041, AC tests: crates/synveda-store/tests/workspaces.rs (21) + crates/synveda-gateway/tests/workspaces_api.rs (23) + crates/synveda-gateway/tests/openapi.rs (5) + the CPR-4 block of crates/synveda-store/tests/rls.rs (5), demo: demos/cpr-4-workspaces.sh. Prompt 4, and the programme's **first public surface**. CPR-3 built the scope substrate and gave it no API on purpose; two of its five shapes — `workspace` and `project` — were named by the vocabulary and had nothing behind them. This puts something behind them: `workspaces`, `projects` and `project_repositories` as product-level subtypes, each owning one scope **created in the same transaction as itself**, so the outcomes are both and neither and there is no compensating delete anywhere. The tenant root is minted by the first thing that needs a parent, from the `tenants` row and never from `hierarchy_nodes` — which is what makes a person's first act `POST /v1/workspaces` rather than a request to declare an organisation. The judgement worth reading is ADR-0070 decision 2 applied one level up: the rules holding a subtype and its scope together are **foreign keys**, including the one nobody would think to make structural — because the composite key carries `slug`, a workspace and its scope are *one name*, so a product path and a scope path cannot diverge. **Twelve routes**, and two mechanisms that exist because this plane's first callers are retrying agents rather than people clicking buttons: creation takes a **required** `Idempotency-Key` (same key + same request replays with 200, same key + different body is 409, and the race between the lookup and the insert **replays rather than conflicting**, because that caller is the timeout retry the whole thing exists for arriving early) and update takes a **required** `expected_revision` (a mismatch is 409 and writes nothing, and monotonicity is a trigger rather than store code, because a precondition is worth nothing if the number can be rewound by anything holding a connection). Three findings from building it. **A replay must still take the PDP decision** — a caller whose binding is revoked between the attempt and the retry is refused, and a replay that skipped the decision would be a cached authorisation; there is a test that revokes the binding between the two calls. **Onboarding state is the server's answer, not the client's inference**: "no workspaces exist" and "you may read no workspaces" are different facts with the same shape, so `/v1/me` computes `blocked | needs_workspace | needs_project | ready` rather than leaving twelve clients to guess. And **a filesystem path is never a repository identity** — canonicalisation collapses transport, credential, port and `.git` so four ways of writing one repository are one row, and a path is refused *by name* in the type layer with a message and by a CHECK behind it, because a path differs per machine and changes when somebody moves a directory. Dropping the credential turns out to be load-bearing rather than tidy: it is what makes `canonical_uri` safe as an audit payload, and there is a test that pastes a `ghp_` token into a remote and sweeps the chain for it. Two things stated rather than implied: **every decision is anchored at the tenant**, because the Cedar entity model still materialises `Scope` from `hierarchy_nodes` and a generic scope has no row in it — the six new actions already apply to `[Tenant, Scope]`, so Prompt 5 re-anchors them with a route change and not a contract change, and this is the largest thing the feature defers; and the **OpenAPI document covers twelve operations out of a `/v1` surface with sixty-six**, which is a bounded start the document's own description states so nobody reads its silence as coverage. That document is the other half of the feature: `utoipa` derives it from the handlers, a test fails when the committed file and the tree disagree or when a documented path is not mounted, and `console/src/generated/api.ts` is generated from it — three artefacts, two checks in `make ci`, and the only one a human edits is the Rust. **No console shell yet** (Prompt 20), and the generated types typecheck without a screen consuming them.
- [x] [CPR-5: Membership, groups, grants & invitations](CPR-5.md) — done 2026-08-18, ADR-0072, migration 0042, AC tests: crates/synveda-store/tests/access.rs (30) + crates/synveda-gateway/tests/access_api.rs (17) + crates/synveda-policy/tests/access.rs (7) + the CPR-5 block of crates/synveda-store/tests/rls.rs (5), demo: demos/cpr-5-access.sh. Prompt 5. CPR-4 gave the platform workspaces and projects and left one thing conspicuously absent: **nobody was in them**. A workspace had a name, a governed scope and no members, and the only way anybody could act on it was a role binding on a node of the **old** hierarchy — the tree Prompt 6 deletes whole. `groups`, `group_members`, `scope_grants` and `pending_invites` are the membership model, in one shape for a person working alone and for a company with a directory: a grant gives a **subject** — a principal or a group — a **role key** at a scope, and the scope's subtree inherits it. Creating a workspace or a project now mints an `owner` grant for its creator in the creating transaction, because a collaboration space nobody is a member of is not one. The judgement worth reading is what is **not** here. There is **no permission table**, and ADR-0072 decision 2 forbids one arriving: six role keys and nothing anywhere saying what any of them may do, because the Cedar packs decide that and a second mapping would be a second decision point that disagrees with the first the day somebody edits one. Inheritance is the **scope tree** rather than a fan-out — a workspace grant reaches every project inside it through `scope_closure` at read time, and a test asserts **zero rows** are written at the project to make it so, which is what removes the derived set that could go stale. The one place the walk stops is a `principal`-shaped scope: nothing above it reaches in, asserted against a grant at the tenant root — the widest thing this model can express. Three findings from building it. **A principal had to be a token subject rather than an identity row**: ADR-0015 decision 2 already argued the general case, but the case that decided it is that an `identities` row in this tree still needs a `hierarchy_nodes` node, so a membership model keyed on it would have needed the model it replaces — a synchronisation between the two, which this programme forbids outright. The cost is that a member list carries subjects rather than names, and the identity plane is where that is fixed. **An invitation is a bearer credential and is built like one** — the provisioning credential's shape (ADR-0059 decision 13): 256 bits, the whole string hashed so a secret lifted behind another tenant's prefix hashes to nothing, a greppable prefix so a leak is findable, and the plaintext in exactly one response beside a copyable URL, with no email delivery anywhere in the product. Three consequences fall out and each is a decision: the token is in a URL path, so the request span records that route's **pattern** rather than its URI (a trace is an ordinary log); a replayed creation is a **409 saying the token cannot be re-served** rather than a 200 with the field missing; and redeeming takes no `Idempotency-Key` because a one-time token already is one — a retry by the same principal replays, a second person is refused. There is a test that sweeps the whole audit chain for the secret. And **redeeming needs the token rather than a role**, under every pack: a person this product invited and then turned away for want of a binding is the failure that permit exists to prevent — expressed as an *action* rather than an exemption, so the invariant floor still refuses a quarantined principal and (because it is tenant-only) every service identity. Two things stated rather than implied: **grants are not yet a PDP input** — the resolution is implemented and served, but the role keys do not reach Cedar, because `context.roles` still carries the old role-binding vocabulary over the old hierarchy and feeding a generic-scope grant into a tenant-anchored decision would both translate between the two models and silently widen every tenant-wide decision by every project-level grant; and **every decision here is still anchored at the tenant**, inherited from ADR-0071 decision 3. The four new Cedar actions already apply to `[Tenant, Scope]` where the model admits it, so the re-cut is a route change and not a contract change — the second time the same debt is written down rather than the first time it is hidden.
- [x] [CPR-6: Governed scope anchors — the PDP re-cut](CPR-6.md) — done 2026-08-19, ADR-0073, migration 0043, AC tests: crates/synveda-policy/tests/anchors.rs (17) + crates/synveda-store/tests/anchors.rs (13) + crates/synveda-gateway/tests/anchors_api.rs (9) + the CPR-6 block of crates/synveda-store/tests/rls.rs (2, 81 → 83) + the directory projection in crates/synveda-gateway/tests/scim.rs (13 → 14), demo: demos/cpr-6-anchors.sh. Prompt 6. CPR-3, CPR-4 and CPR-5 each built a piece of the governed scope model and each recorded the same debt in its own ADR: **the decision point still described the old hierarchy**. Twenty-six routes anchored every decision at `Resource::Tenant` because a governed scope had no chain in the Cedar entity graph, and ADR-0072 decision 3 went further — the role keys a grant carries were stored, resolved and served, and **never reached Cedar at all**. So the product had a complete membership model that decided nothing: a workspace `owner` grant was a governed record of authority, and what actually let somebody administer a workspace was a role binding on the tree this programme is deleting. This is that cut. The **scope-anchor resolver** answers "where does this request stand" from six inputs — the caller's own scope, the selected project, the selected workspace, the organisation-unit relationships above them, the tenant root, and every scope a direct or group grant names — and returns an **ordered set** rather than the one fixed organisation chain the old model assumed. That assumption was wrong in two ways at once, and the second is the one worth reading: a placement chain runs **upward**, so being *in* a team made you a member of the department and the org, while a grant runs **downward** — which is why "a project-only grant does not reach its workspace" was a sentence the old model could not make true. Anchors are therefore deliberately **not** entity parents of the principal: an entity parent is the upward direction, and a project grant that made its holder a member of the workspace above it would be the placement chain returning under another name. Seven Cedar entities — `Tenant`, `Scope`, `Principal`, `Group`, `ScopeGrant`, `Workspace`, `Project` — each of the four new ones parented to the scope it belongs to, so a decision names the thing it is about and every containment rule already written over scopes reaches them without being restated. Revoking now names **the grant**, which is what makes "who may take away a directory-managed grant" a sentence a pack can write. Four findings from building it. **The privacy floor was in the wrong place and was strictly too strong**: every pack restated `resource.kind != "user"` on every permit, which made "share my own notes with you" unsayable even by the person whose notes they are. It is now one base-layer forbid over `principal`-shaped scopes that no pack can drop, with `principal.private` — your own scope, plus any somebody granted you **directly** — as the only door, and a short closed governance carve-out that deliberately excludes `RoleAssign` and `MembershipGrant`, because a grant written at a private scope puts that scope in the grantee's `private` set and an administrator who could write one could grant themselves everybody's notes. **`standard` needed a real replacement for the department, not a redundant one**: the obvious re-cut (`resource in principal.anchors`) turned out to be dead policy, because a grant reaches its own subtree under *every* pack — what makes `standard` different is one step **outward**, so it reads `principal.ambit`, the parent of every scope you hold, minus the tenant root. **A principal scope had to name its subject in a column** rather than by a slug convention or an `attributes` entry, because the resolver reads it and ADR-0070 says in its own header that `attributes` is never an authorisation input. And **the ownership check now runs before the decision** on every per-object route, because deciding about a workspace requires having fetched it — which is ADR-0012 decision 7's order and the hierarchy plane's, so it is a convergence rather than a new rule. Two costs stated rather than implied: the anchor resolution is **six extra indexed reads on every `/v1` request**, including the hierarchy planes where it can never contribute, and the obvious optimisation is deferred because a cache in front of the thing that makes revocation immediate is the wrong first one; and **`standard`'s golden matrix moved** — a caller with no grant now reads their own chain where before they read their department, which is the removal of the rank recorded in the golden tests rather than accommodated. The old hierarchy APIs stay until the prompt that deletes them and are no longer *required* by PDP evaluation: they project their rows into the decision point's one scope vocabulary at the caller's edge, a single function that writes nothing and goes with `hierarchy_nodes`.
- [x] [CPR-7: The hierarchy cutover — one scope tree](CPR-7.md) — done 2026-08-20, ADR-0074, migrations rewritten in place (the scope substrate at `0004`, `0009_role_bindings` and `0040_scopes` deleted, epoch 1 → 2), AC tests: crates/synveda-gateway/tests/admin_scopes_api.rs (7, incl. the negative half: every `/v1/hierarchy` route 404 and every old scope kind fails validation by name) + the re-cut suites (store/policy/gateway) + demos/cpr-7-scopes.sh. Prompt 7, and the one six records deferred to: the prompt that deletes the old hierarchy **whole**. `hierarchy_nodes`, `hierarchy_closure`, `role_bindings` and `group_mappings` leave the schema; the rank vocabulary (`org`/`division`/`department`/`team`/`user`, `rank()`, the child-outranks-parent rule, the root-must-be-an-org CHECK) leaves the types; `/v1/hierarchy/*`, `synveda hierarchy`, `synveda role bind`, the placement-based quarantine convention, the `synveda-{dept}-{team}` JIT convention, the HIER-2 chain cache and the console's hierarchy explorer leave the product — replaced by **six public admin routes** over governed scopes (`/v1/admin/scopes` list/create/get/patch/ancestors/descendants), **five operator CLI commands** (`synveda scope list|show|create|move|tree`), and pack assignment plus the VedaFlow curator file re-homed under the same prefix. Three design decisions worth reading. **Placement is identity, not convention** (ADR-0074 decision 3): an identity's scope is its own `principal`-shaped scope, minted in the provisioning transaction for users, services and directory identities alike — a directory user's keyed by its `externalId`, adopted at first login through the existing correspondence rule — and "unmapped" means *ungranted*, decided per action by the anchor model and the privacy floor rather than per person by a placement-derived flag. **The admin door is a grant** (decision 4): the `synveda-admins` convention now upserts an `administrator` grant at the tenant root at every login — the ADR-0015 decision 6 discipline on the new noun, and the operator door ADR-0073 recorded as missing (a tenant's first admin-group login mints its first grant, no break-glass needed). **A move is decided twice** (decision 5): once at the scope being moved and once at the destination, audited with both ends, because authority at one end of a move is exactly half the authority the act needs. The schema epoch bumped to 2 so every pre-cutover database is refused with the reset instruction by the CPR-2 guard rather than by a migration checksum error, and the chain rewritten in place rather than translated — no data migrates, by the locked decision. The approval matrix, proposal approvals, curator files and every Cedar role list speak **grant keys only**: `steward`/`org-admin`/`compliance` collapsed into `administrator`, `security-reviewer` into `reviewer`, with the two invariant floors re-vocabularied (restricted ⇒ administrator + 2 distinct; any skill ⇒ reviewer + 2 distinct) and Prompt 27 named as the re-cut that restores specialist names. Three consequences recorded rather than implied: the HIER-2 warm cache went with its tree (chains resolve per request through `scope_closure`; inject's p50 budget absorbs one indexed read, and ADR-0073 decision 7's deferred anchor-cache is the optimisation if measurement ever demands one); **group-driven placement is gone** — a directory user stays at their own scope and belonging is directory groups + grants, so ADR-0059 decision 10's move-sealing logic and the reconciler's `apply_placement` are deleted rather than re-expressed, with placement-as-configuration owed to Prompt 29; and the console explorer's roles panel is gone with the bindings (the tree, packs, lapses and capabilities panels re-cut onto the scope plane; the access plane's own screen is a later console prompt's). **Three things this feature found by looking rather than by failing, and fixed.** The scope-kind re-vocabulary of the approval matrix dropped `tenant` out of both cells — so `regulated-strict` priced a memory at an org unit at curator + administrator and the same memory at the **tenant root**, the widest audience the product has, at nothing — and the partition test that exists to catch exactly that ("no cell falls through to auto-approve by accident rather than by decision") had had `Tenant` removed from the shapes it iterates, while `cross_scope.rs` had been rewritten to assert the hole. `SHARED` is `[Tenant, OrgUnit]` again, the test iterates all five shapes, and the golden matrix carries the restored cells. The `synveda-{dept}-{team}` convention was recorded as deleted in three documents while `synveda_identity::mapping` still exported its parser and slug helper — dead the moment placement became identity, and invisible to `dead_code` because they are `pub` in a library crate. And a move was "audited with both ends" while its event carried only the destination; it now carries `moved_from` too, asserted by id. **Standing, and it is the big one:** a grant does not yet widen what a session *composes* — `composition_plan` still walks the caller's chain (their own scope outward to the tenant root) and anchors reach it only as decision context, so with placement gone an agent's `inject` sees its own scope and the tenant root and nothing else, and joining a workspace gives that session nothing. Making the candidate set the anchor set is the composition contract's re-cut, owed to Prompts 16–18; every fixture here was re-cut to the model as it stands. Also standing: four demos whose subject is the deleted model go with it (`hier-1`, `hier-2`, `hier-3`, `authz-3`; 69 → 65) and the programme's own `cpr-4`/`cpr-5`/`cpr-6` are re-cut onto the grant bootstrap, but **forty-three Phase-3 demos still seed through `role bind`, `hierarchy_closure` or `/v1/hierarchy` and will fail at that line** — each belongs to a subsystem a later prompt re-anchors, no CI target runs them, and the number is written here rather than left to be discovered.
- [x] [CPR-8: The console product shell & first-run onboarding](CPR-8.md) — done 2026-08-21, ADR-0075, no migration, AC tests: console/src/{routes,client,cache,selection,onboarding}.test.mts + console/src/{shell,people}.test.tsx (**72 new; 49 → 121**) + the OpenAPI contract test. Prompt 8, and the prompt that makes six prompts of platform reachable from a browser. CPR-3 through CPR-7 built governed scopes, workspaces, projects, membership, invitations and the re-cut decision point — all of it CLI-only, while `App.tsx` still resolved its session with `whoami` and mounted the proposals inbox and the scope explorer one after the other with **no navigation at all**. That is a governance console: the right first screen for somebody reviewing other people's publications, and the wrong one for the person who just installed the product, who has no proposals, one scope, and no way to create the workspace the product now asks for first. What replaces it: a **route-based product shell** whose **primary** menu is the product (Home, Sessions, Knowledge, New Learnings, Skills, Tools, People, Settings), shown to everybody unconditionally — because a navigation that grew and shrank with a role would teach every reader a different shape for one application — and whose **advanced** menu is governance (Reviews, Scopes, Policies, Audit, Service identities), shown only where the caller's capability forecast offers the plane. Beside it: workspace and project switchers over a selection persisted per browser and **reconciled** against `/v1/me` on every load (a preference that still exists wins; otherwise the first thing that does; a project that belongs to another workspace is dropped); one query/cache layer and therefore one loading state and one error state per route; a **People** page that answers *why* — workspace members, project-only members derived from each row's own `inherited` flag rather than by diffing two lists, pending and settled invitations, each row naming its role, access source, the scope its grant is actually written at, the group it came through and whether a directory owns it, with invite/revoke/remove **absent rather than disabled** where the API would refuse; and **first-run onboarding** (workspace → project → repository → agent client → connection instructions → connection check) driven entirely through the product's own public routes, with no bootstrap path anywhere. Four decisions worth reading. **The personal/team question seeds and does not brand** (ADR-0075 decision 6): it chooses the policy pack assigned at the new workspace's scope and whether a group and invitations are set up, and records nothing — no column, no field, nothing that branches — because ADR-0068 decision 1 forbids an edition and a wizard asking "is this just you?" is the friendliest possible door for that branch to arrive through; `onboarding.test.mts` asserts the plan carries no `kind`/`edition`/`tier` field by name. **The seeding is best-effort and reported**: a first caller holds an `owner` grant on what they just created and may hold nothing that permits `policy.assign`, so a refusal renders as a sentence pointing at Advanced ▸ Policies and never blocks the wizard — silently skipping it is the one unacceptable option, because somebody would leave believing their workspace was governed the way they chose. **The connection check claims only what a browser can prove**: the console's own CSP confines it to this origin, so it verifies the gateway answers this reader and the chosen project is readable, and says on the page that it cannot see whether the agent client on the reader's machine is installed or holds a credential. And **a plane with no API gets an honest page rather than an empty list** (decision 7): Sessions, Knowledge, New Learnings and Tools are in the primary menu because they are what the product *is*, none of them has an API at this commit, and each says what will be there and which prompt delivers it — an empty list would be indistinguishable from a plane that works and had a quiet week. **The generated contract got stricter, and found a defect doing it.** `list_scopes` declared its `parent_id` as a **path** parameter on `/v1/admin/scopes`, a route with no such placeholder — invisible while every caller was hand-written, and the first thing a generated client trips over; fixed at the source (`#[into_params(parameter_in = Query)]`) with the document and types regenerated. The generator now also emits the **runtime** path/method table beside the type table (so no hand-written second copy of a path exists anywhere) and an `idempotent: true` flag for the eight operations whose document requires an `Idempotency-Key`, which the client turns into a **compile-time obligation** — and refuses at runtime for an untyped caller, both ways round. **Two defects the new tests found in this feature's own code, before it shipped:** `slugFrom` ran NFKD and then treated the combining mark as a separator, so "Ünicode Name" proposed `u-nicode-name`; and `accessSource` replaced the group clause with the directory clause when both were true, dropping the actionable half — "managed by your directory" says you cannot change it here, and *which group* says what to change instead. **Standing:** four primary pages have no plane behind them (Prompts 9–15); seven surfaces still call hand-written paths in `api.mts`, grouped and labelled, until Prompt 19 puts the rest of `/v1` on the contract; there is no group-management screen (it belongs beside the directory adapter, Prompt 29); and rendering is still asserted with `react-dom/server` + `toText` rather than a browser runner, so what is covered is which facts appear and not the switchers' `onChange` or the wizard's transitions — those are asserted at the seam below them. No npm dependency was added: routing and the cache are written here because the shipped bundle's licence gate has no exception mechanism and the page is served under `default-src 'none'`; `check-npm-licences` reports 3 shipped packages before and after.
- [x] [CPR-9: The foundation audit — hardening the scope and access cutover](CPR-9.md) — done 2026-08-22, no ADR (no locked decision reopened), no migration, AC tests: crates/synveda-gateway/tests/foundation_audit.rs (6), the widened crates/synveda-store/tests/epoch.rs::no_old_to_new_data_migrator_exists, and the two wire-shape pins (crates/synveda-cli/src/login.rs, crates/synveda-gateway/src/auth.rs, crates/synveda-gateway/src/capabilities.rs). Prompt 9, and the first of this programme asked to **audit** its predecessors rather than build on them. Prompts 1–8 each shipped green, and each suite proved its own plane worked; none had asked the question that spans them — what does a caller learn, or fail to learn, that their grants do not say? The new suite asks it with **valid identifiers from another tenant** (a real workspace, project, scope, group, grant and invitation, minted by that tenant's own administrator) and asserts each is **indistinguishable from an id nobody ever minted** — same status, same error kind — because a caller who can tell the two apart can enumerate another tenant's inventory a uuid at a time; with a second workspace inside **one** tenant, where RLS is silent and only the PDP and the anchor resolver stand; and with somebody else's `principal` scope probed by the tenant administrator, the one caller who reaches everything else. It watches the three channels a denial leaks through when the status code is right: counts, error text, and the navigation capabilities the console builds its menu from. **Three defects, all cutover residue.** (1) *A grant at a workspace did not reach the listings.* `GET /v1/workspaces` and `/v1/me` took one decision at the tenant root and applied it to every row, so a caller granted `member` at a workspace — who holds nothing at the root — got an empty list, `workspace_count: 0` and `onboarding.state: needs_workspace`, while the `anchors` block of the very same response said `workspace.read: true` at that workspace: two answers to one question in one payload, and the console renders both, so an invited member was sent to the first-run wizard to create the workspace they had just joined. Listings now decide **per row against the row** — one gather, one entity batch, one Cedar evaluation per row under that row's own chain and assignments, shaped after `capabilities::at_anchors` — with **no fast path** for a caller permitted at the root, because "permitted above ⇒ permitted below" is not a property Cedar has. The outsider's 403 is unchanged. (2) *Two client/server contracts had drifted apart*, both on routes Prompt 19 has not put on the contract: `synveda login` still required the `identity.quarantined` CPR-7 deleted, so **every login failed to parse the session it had just been handed** — after the browser round trip, after the code exchange, with the credential already minted; and `synveda whoami --capabilities` still read the `roles`/`role_assign` shape CPR-7 renamed and dropped, so the flag could not parse a single response (plain `whoami` shares the route and never asks for the block, which is why nothing noticed). Both fixed and **pinned from each side**, each test naming the other's file. (3) *The no-data-migrator guard checked one file of forty-one* — the file a translator written today would live in, leaving unchecked the forty where ones written before the cut already were. It now scans the whole chain, skips dollar-quoted function bodies so an audit trigger is not mistaken for a translation, and pins the three inherited pre-epoch upgrade statements by name; a fourth fails the build. They are not deleted and the test says why: they are unreachable (a pre-cut database never reaches the migrator, so the only databases that run migrations 8 and 38 are fresh ones with those tables empty), and deleting them would trade the guard's reset instruction for a checksum error on every existing epoch-2 database — an epoch bump and a global reset to remove statements that cannot run. Prompt 33's squash removes them. **Verified sound, independently of the suites that assert it:** 52 tenant-bound tables all `ENABLE` + `FORCE` with a policy and the four hierarchy tables absent; the closure holding over ~23,000 scopes (self row per scope, no cross-tenant pair, no cycle, a distance-1 edge per parent pointer, one parentless `tenant`-shaped root per tenant, `principal_id` exactly on principal scopes); the privacy forbid holding against a tenant administrator; the capability probe serving no `scope_path`, `pack` or roles at a scope the caller holds nothing at; an invitation surviving a cross-tenant redemption attempt still spendable by its recipient; and the console's `offersRoute` failing closed on a missing key. **Residue classified:** the three defects were the only unjustified matches in production code. Stale prose was corrected where a reader would act on it — `synveda init` told operators to run `synveda hierarchy list` and claimed the first login binds `org-admin`, `synveda whoami` pointed at `synveda hierarchy capabilities`, and `ScopeId`'s own doc comment still defined a scope as a rung of the deleted rank ladder. The rest is prose narrating the deletion, negative tests asserting the 404s, and test-local variable names. The 43 Phase-3 demos still seeding through `role bind`, `hierarchy_closure` or `/v1/hierarchy` are unchanged and were **counted again rather than trusted**: CPR-7's number is exactly right.
- [x] [CPR-10: The session ledger and runtime API](CPR-10.md) — done 2026-08-23, ADR-0076, migration 0044_sessions, AC tests: crates/synveda-gateway/tests/sessions_api.rs (**14**), crates/synveda-store/tests/rls.rs (5 new), 17 unit tests, demo: demos/cpr-10-sessions.sh. Prompt 10, the first of Stage B, and ADR-0068 decision 5 in full: **what an agent does becomes a governed record**. What it replaces is a string — `observe_events.session_id`, an opaque harness identifier with a length CHECK and nothing else, copied into an audit payload by `/v1/inject` and `/v1/recall` and read back by nothing. Four consequences, each a thing this product claims to do and could not: a run an agent only *read* in **did not exist** (ADPT-8 measured it — a headless Claude Code run: three sessions, three `inject.ok`, **zero** `observe.done`, exit 0); a run could not be governed, because there was no resource for the PDP to decide about; a run could not be retained, ended or audited; and the console could not show one, which is why CPR-8 had to render a placeholder on the page it put first in the primary menu. **Three tables**: `sessions` (its workspace, optionally its project, the derived governed scope, who opened it, the client/version/installation, the harness's own id, the agent, the model, the repository and branch, a task summary, a five-state lifecycle and a bounded metadata bag), `session_events` (immutable, append-only, ordered, idempotent by the client's own `client_event_id`, over a closed twelve-name vocabulary spanning lifecycle, messages, tools, files, commands, skills, context requests and adapter warnings) and `session_context_runs`. **Seven routes**, on the OpenAPI contract from the day they exist (32 → 39 operations) — open, list, get, append events, end, timeline, and `POST …/context-runs`, whose *shape* is final and whose depth Prompt 18 fills in without changing it. **Two Cedar actions** (`SessionRead`, `SessionWrite`), a `Session` entity parented to the scope it runs at, permits in all three packs (**@18 → @19**), four audit action types, and the console's Sessions page — the first of CPR-8's four planned pages to get a plane behind it, driven entirely through the generated client. **Six decisions, all in ADR-0076.** *The governed scope is derived, never submitted*: three columns and two composite foreign keys hold `scope_id = coalesce(project_scope_id, workspace_scope_id)` as a row-local fact, because a client that could name the scope could name one its workspace is not in. *Five states, because the close is two-phase*: an adapter learns a run is over at a hook that must return quickly and usually still has events buffered, so `ending` means *no new work, I am flushing* and still accepts them — and `abandoned` (nobody closed it) stays apart from `failed` (it broke) because the two call for different things. *No revision and no `expected_revision`*, stated rather than skipped because the plane's rule is otherwise uniform: a precondition stops a lost update and ending has one target state, so two concurrent ends are one transition and one refusal that already names the state the run is in. *Two idempotency mechanisms, not redundant*: opening and composing take the header, appending is idempotent per **event**, because a redelivered batch overlapping a previous one by three of ten must append seven and report `duplicate` for three at their original positions — which a request-level key cannot express. *Nothing on the wire carries a tenant or an acting principal*, and a body naming one is **refused rather than ignored**, because a server that silently dropped the field would behave correctly and teach every client author that it works. And *the timeline is a projection* over the two tables and never a third — the demo asserts no `%timeline%` table exists — because a materialised transcript is a second copy of `session_events` that disagrees the first time one is written and the other is not. **A listing decides per row against the row** from the start rather than retrofitted (CPR-9's rule), bounded at 500 candidates newest-first with `truncated` on the envelope — a different thing from the cap CPR-9 refused, because this is a recency feed of an unbounded table where "the most recent N" is a well-defined answer. **One defect this feature's own tests found in its own code**: `SessionEventType` derived `rename_all = "snake_case"` beside an `as_str` of `message.user`, so the API answered with one spelling and refused the other; every event-appending test failed at once, and the fix is per-variant renames with a unit test pinning that the two agree. **The old routes are untouched and nothing bridges them**: `/v1/observe`, `/v1/inject` and `/v1/recall` still take the old string, nothing reads one to populate the other in either direction, and Prompt 11 re-cuts the observe path and deletes it.
- [x] [CPR-11: The session product experience](CPR-11.md) — done 2026-08-24, ADR-0077, migration 0045_session_end_reason, AC tests: crates/synveda-gateway/tests/sessions_api.rs (**7 new**, 21 total), console/src/sessions.test.tsx (**9**), console/src/sessions.test.mts (12 new), console/src/routes.test.mts. Prompt 11. CPR-10 made a run a **governed record**; this makes it a record somebody can **use**, and the distinction is not a polish item — every one of CPR-10's fourteen acceptance tests asks whether a run is correctly stored, decided and chained, and none asks whether a person with a question can get an answer. **Four holes.** (1) *A run older than one answer was unreachable.* The listing said `truncated: true` and could not say **where to continue**, so a deployment whose agents open a few hundred runs a week had no API path to last Tuesday's run — not a slow one, none — and nothing narrowed by who ran a thing, which client, or when. (2) *A timeline reported one clock.* `received_at` has been stored since migration 0044 and served by nothing, while the adapters this product ships spool to disk when the gateway is unreachable and flush later: an hour of a transcript arrives at once, an hour late, and a reader sees a perfectly plausible transcript with no sign that any of it was recovered. (3) *A payload could neither be read nor be withheld.* It was echoed to the client that wrote it and served by no route, so the forensic question had no answer — and the moment one exists it is the largest disclosure on this plane. (4) *A run said how it stopped and never why.* **What it adds:** keyset pagination (`cursor` in, `next_cursor` out, `truncated` **deleted** rather than kept beside it) plus `client_name`, `principal_id` and a half-open `started_after`/`started_before`; `received_at` and a server-computed `delayed` on every timeline event entry; `GET /v1/sessions/{id}/events/{event_id}` behind a **new Cedar action** `SessionDiagnostics`, strictly narrower than each pack's own `SessionRead` (**packs @19 → @20**, no existing permit changed); `sessions.end_reason` set at close and carried into the chain; and the console's product surface — a filter bar, Load more, a **route per run** at `/console/sessions/{id}`, an ordered timeline showing both clocks and marking what did not arrive live, a warning banner and per-entry marks, repository and branch resolved through the project's own attachments, and a payload expansion offered from the caller's forecast **at that run's scope**. 39 → **40** operations on the contract. **Two decisions worth reading twice** (ADR-0077). *The cursor follows the last candidate a page considered, not the last row it served*: rows are decided one at a time **after** they are scanned (CPR-9's rule), so a cursor on the last served row would end the listing whenever a whole page was denied while readable rows sat below it — which is why a page may be **empty and still carry a cursor**, and the schema says so rather than leaving a client to discover it. *Lateness is one flag and not three*: a locally spooled batch, a replay after a crash and a machine with a wrong clock produce the same two instants, so the server reports the **gap** and refuses to name a cause; a console that labelled the third “recovered from a local spool” would be inventing a distinction the data does not contain. **The payload split is asserted with one caller holding one half and not the other**: a `member` at a project reads that run's timeline and is refused `session.diagnostics` by name on the same run, and the timeline they read carries no payload text at all — the assertion that fails the day somebody puts one on an entry “for convenience”. The chain records **which** event was expanded and never what was in it, because an audit log that copied every prompt somebody read would be a second, unbounded transcript store with weaker access rules than the first. **What it does not carry:** the observe re-cut. `/v1/observe`, `/v1/inject` and `/v1/recall` are still untouched and `observe_events.session_id` is still a string — §9's Prompt 8 work, unstarted, and the divergence is recorded in the programme's own record rather than absorbed. **One test is red at this commit and it is named rather than hidden**: CNSL-2's explorer parity corpus is a *recording* of what the capability probe serves and now has `session.diagnostics` in it — the same expected re-record CPR-10 hit for `session.read`/`session.write`. `SYNVEDA_RECORD_FIXTURES=1 make db-test` closes it in one command. It is open because the Docker daemon on the machine this ran on wedged partway through and took the Postgres container with it (the port kept accepting TCP and the backend stopped answering), so the DB-backed figure — **571 passed, 1 failed** — comes from running the suite directly against a freshly migrated database rather than through `make db-test`. The fixture was **not** hand-edited to the bytes it will have: a corpus written by hand is the drift that test exists to catch. `make ci` itself is green.
- [x] [CPR-12: Durable Claude session delivery](CPR-12.md) — done 2026-08-23, ADR-0078, migration 0046_session_ingestion, AC tests: crates/synveda-cli/src/spool.rs (**13**), crates/synveda-gateway/tests/{sessions_api,session_ingest_load,session_redaction,context_runs}.rs, crates/synveda-eval (159), adapters/claude-code (87). Prompt 12. CPR-10 and CPR-11 built the record and made it usable, and **nothing wrote to it**: eleven routes and four tables, while every observation this product actually received still arrived at the global `POST /v1/observe` under a `session_id` string the adapter invented, and every block it composed still came from `POST /v1/inject`. Two planes claiming to answer "what has this agent been doing" is not a migration in progress, it is a product with two answers. **What existed before it:** the adapter's delivery was durable in name only — `flush.mts` appended to a spool and fired on Stop, and a delivery that failed wrote **nothing back**: no attempt count, no last-attempt instant, no acknowledgement state, no sequence, so a later hook could not tell what had been sent from what had not and the only correct thing it could do was nothing. An offline laptop lost the session. **What it adds:** a durable local spool with a versioned format — spool version, installation id, session id, client event id, sequence, event type, occurred time, payload, payload hash, delivery attempts, last attempt time, acknowledgement state — written temp-then-fsync-then-rename so a kill mid-write leaves the old file or the new one and never half of either, and **never read in its predecessor's format**; four hooks that own delivery (SessionStart opens or resumes and retries the backlog, Stop records fast and delivers, PreCompact records, SessionEnd flushes inside 3s) with three CLI commands that only diagnose (`synveda session flush`, `spool status`, `spool purge --acknowledged`); and context injection as a context run. **The cutover:** `/v1/observe`, `/v1/inject` and `/v1/recall` deleted with their handlers, DTOs, `observe_events`, the `observe` queue, `observe_quarantine` and `ObserveKind`; extraction re-anchored on `session_events`; `synveda recall`, `synveda mcp` and the eval harness re-pointed. **Three decisions worth reading twice** (ADR-0078). *A memory lands at the scope the run was decided at*, not the submitter's home — which is the difference between a workspace remembering what happened in it and every agent accumulating a private pile, and it is only possible because a run derives its scope from a workspace. *The spool hashes with SHA-256 and not BLAKE3*, because the thing that must verify it is Node and Node has no BLAKE3; a chain hash and a spool checksum answer different questions and sharing an algorithm would be the only thing they had in common. *Only acknowledged events may be automatically deleted*, so `purge` requires `--acknowledged` and offers no `--all` — a command that can delete undelivered observations on a typo is a command that will. **The event-loss boundary is documented rather than closed** (decision 8): a host that dies before any lifecycle hook runs takes the un-flushed tail with it, bounded by one turn because Claude Code fires Stop at the end of each — closing it means writing on every token, which is a different product with a different cost. What this design does guarantee is that **nothing which reached the spool is ever lost**. **Two performance findings.** The append seam missed MEM-1's <20ms ack budget at 35.6ms after the move; the first hypothesis — a per-session row lock — was **wrong**, and spreading load across runs moved the median 0.3ms. The cause was per-row inserts, and three statements (pre-filter existing ids, one batched `unnest` insert with contiguous sequences, read back) put it at 17–19ms. **And a pre-existing security hole was closed**: CPR-11 shipped a unit test asserting a timeline summary **is** the message text and an integration test asserting a timeline carries no payload text — two tests that could not both pass, and the one that was passing was the wrong one. Messages now summarise as type plus length. **What it left standing:** the extraction, security and QA-index eval suites enumerated a corpus through `/v1/recall`'s sweep, and a context run cannot stand in — it ranks and budgets where a sweep enumerates, so what it left out would be a property of the budget rather than of extraction. They **fail by name** with the reason rather than measure a different question; Prompt 18 re-cuts recall and Prompt 32 re-measures. And CPR-12 filed **CPR-13** after finding that 45 of the 67 shell scripts under `demos/` were dead — see its entry below.
- [x] [CPR-13: The demo corpus re-point](CPR-13.md) — done 2026-08-24, no ADR, migration, API or runtime change. The final generated-contract inventory found 49 affected scripts, not the earlier route-name estimate of 43: 18,528 copied lines of retired hierarchy/role binding, record, global runtime and hand-written HTTP setup became concise feature-specific current-scope/session/capture/Knowledge/context narratives over one isolated epoch-2 Postgres harness (18,528 affected-script lines became 504 plus a 52-line helper; net reduction 17,972). `scripts/check-demos.mjs` recursively checks all 73 shell scripts against freshly built recursive Clap help and generated OpenAPI, recognises binary aliases, and excludes comments, explanatory output, heredoc fixtures and the external IdP contract. Four tests deliberately inject dead command/path/alias cases; `make check-demos` is in `make ci`. Live-Postgres representatives pass: MEM sessions 22/22 plus 10k load 1/1 at 1,006 events/s, CTX 1/1, FLOW 4/4, AUTHZ 2/2 and ADPT authentic-frame 2/2. The ADPT result is replay through the real built hook and current gateway; CPR-14 remains the distinct genuine Claude Code 2.1.241 live-client evidence. Complete `make ci` passes; `make db-test` is N/A because no persisted behaviour changed.
- [x] [CPR-14: Live Claude Code session acceptance gate](CPR-14.md) — done 2026-08-24, ADR-0079. The deterministic CI gate runs genuine, versioned Claude Code 2.1.220/2.1.241 frames through the built hook child, public session API, epoch-2 Postgres, PDP, ingestion, timeline and verifying audit chain. It proves outage survival, two pending entries, lost-ack overlap returning `duplicate` at original sequence 5 and `appended` at 6, exactly-once persistence, private spool state and message-free evidence. The separately named live gate used installed authenticated **Claude Code 2.1.241**, installed Synveda plugin **0.2.0** through the supported marketplace path, and verified Claude Code's own enabled-plugin/four-hook/one-MCP report. One real headless Read turn composed one context run, persisted four ordered user/tool/assistant events, flushed at SessionEnd and ended the same run with reason `other`; timeline, diagnostics authority, hashes, acknowledgement state and verifying content-free audit assertions passed. Measurements: client 5,526ms, SessionStart 72ms, local-only Stop 8ms, SessionEnd 53ms, append 28ms, context-run 15ms. The native credential was handed only to disposable isolated config through a private 0600 file and removed. A host killed before any hook writes the in-flight tail remains outside the guarantee.
- [x] [CPR-15: Versioned Knowledge aggregate and provenance](CPR-15.md) — done 2026-08-24, ADR-0080, migration `0047_knowledge`, AC tests: `synveda-types::knowledge` (5), `crates/synveda-store/tests/knowledge.rs` (5) and the dynamic RLS completeness guard; demo: `demos/cpr-15-knowledge-aggregate.sh`. **What it adds:** one stable Knowledge head and immutable revision addresses; all nine types, four origins, six lifecycle states, seven normalised source types and eight append-only relation types; canonical tags and BLAKE3 content hashes; independently governed source provenance; a bitemporal head history; and security-invoker current/version projections with a stored lexical document. Six tenant tables are forced-RLS, tenant-qualified constraints bind every edge, a deferred invariant requires each revision to have a source, and owner-level append-only triggers preserve revision, source-link and relation history. **What it deliberately does not add:** there is no HTTP, CLI or adapter mutation path, Cedar action or audit action in this persistence package, so no caller can bypass the PDP or VedaFlow through it. It neither reads nor writes `records`; ADR-0080 records the exact deletion checklist for the next lifecycle/API cutover. A private source linked to shared Knowledge remains invisible unless its own scope was decided visible. **Evidence:** focused type/store/RLS tests, isolated runnable demo, `make ci` and `make db-test` pass. The persistent development database correctly refused the rewritten epoch-2 chain at removed migration `0009`; the demo uses a disposable database rather than resetting user state.
- [x] [CPR-16: Governed Knowledge mutation lifecycle](CPR-16.md) — done 2026-08-24, ADR-0081, migration `0048_knowledge_lifecycle`, AC tests: `crates/synveda-gateway/tests/knowledge_lifecycle.rs` (3), policy approval/pack/PDP suites and the dynamic RLS completeness guard; demo: `demos/cpr-16-knowledge-lifecycle.sh`. All eight commands create an ordinary VedaFlow proposal with a content-free, hash-bound typed Knowledge effect; permissive policy auto-applies that proposal and regulated policy leaves it for the same review engine. Edit/verify append immutable revisions, supersede and merge write explicit provenance-preserving relations, archive/restore retain content, and forget runs through a reusable durable operation that either records a policy hold or erases aggregate/source/index/command plaintext while retaining only ids and hashes. The gateway no longer starts the old extractor, promotion engine or retention sweeper. The controlled VedaFlow classification write survived only until CPR-17; the remaining old-plane seam is the read-only context composer, scheduled for the explainable context planner. Neither is a Knowledge dual write. `make ci`, `make db-test`, focused tests and the isolated demo pass.
- [x] [CPR-17: Public Knowledge API, search and browser](CPR-17.md) — done 2026-08-24, ADR-0082, migration `0049_knowledge_search`, AC test: `crates/synveda-gateway/tests/knowledge_lifecycle.rs` (public Knowledge case), OpenAPI 5/5, console 151/151, RLS 84/84; demo: `demos/cpr-17-knowledge-browser.sh`. The generated contract now has 53 operations, including thirteen Knowledge operation groups. Current active revisions are cursor-paginated and filterable; lexical search is immediate, TEI supplies the only semantic leg, and deterministic hashes are reported as lexical-only degradation. Every item, source and relation endpoint is decided independently, every mutation calls CPR-16's VedaFlow seam, and audit metadata is content-free. The proposal classify route/CLI/eval call, public proposal `record_ids`/`effect`, record publication/channel aliases, raw-record fixtures and record-oriented public suites are deleted. CPR-18 subsequently removed the non-public session-event extraction write projection; the read-only context composer remains scheduled for the explainable planner. `make ci`, `make db-test` and the isolated demo pass. This package also delivers and subsumes CNSL-4.
- [x] [CPR-18: Session-based capture batches and reviewable candidates](CPR-18.md) — done 2026-08-24, ADR-0083, migration `0050_capture_candidates`, AC tests: `crates/synveda-gateway/tests/capture_api.rs` (3), deterministic Claude lifecycle (2), extractor (64), OpenAPI (5), console (151) and RLS (84); demo: `demos/cpr-18-session-capture.sh`. Explicit and terminal capture freeze one canonical session-event snapshot into a leased, restart-safe batch; extraction re-authorises the session principal, validates/rescans model output and independently decides every Knowledge match before creating reviewable candidates only. Accept/edit, merge and replace enter CPR-16's typed VedaFlow commands, dismiss publishes nothing, and durable decision intent plus child idempotency makes both individual and whole-batch retries converge. Source links are same-session/same-tenant by composite constraints, candidate plaintext needs both session and proposed-destination authority, matches are re-authorised at disclosure, and governed forget scrubs candidate/request plaintext. The PGMQ `session_events` queue and direct record/embed/dedup/link extraction writer and suites are deleted. The generated contract has 62 operations. The isolated demo reports 8 candidates, 8 governed changes and zero old records or queues; `make ci` and `make db-test` pass. The internal record-backed context composer remains read-only, with no Knowledge translation, until the explainable context-planning package.
- [x] [CPR-19: New Learnings lightweight review workflow](CPR-19.md) — done 2026-08-24, no new ADR or migration, AC tests: `console/src/learnings.test.mts` (8) + `console/src/learnings.test.tsx` (6), complete console suite **165/165**. The New Learnings placeholder is replaced by a batch-grouped, project/session/state-filtered workflow over CPR-18's generated contract. Exact source events resolve through the public timeline; raw payload remains behind the run's `session.diagnostics`; every duplicate/conflict/supersession comparison is freshly read through the generated Knowledge detail route. Accept, edit, merge, replace, scope change and dismiss use generated idempotent commands, with exact existing revision preconditions and explicit placement nulls. Only principal/project/workspace anchors forecasting `knowledge.write` enter the picker, while the gateway repeats the authoritative PDP decision. Applied outcomes link to Knowledge; pending outcomes remain outside active Knowledge and link to Advanced Reviews; replace retains history as supersession; dismiss publishes nothing. The comprehensive proposal engine remains under Advanced and no duplicate candidate/review model was added. Production build and `make ci` pass; `make db-test` is not applicable because no database, gateway, policy or persisted behaviour changed.
- [x] [CPR-20: Explainable Knowledge context planning and scoped query](CPR-20.md) — done 2026-08-24, ADR-0084, migration `0051_context_planning`, AC tests: context runs 3/3, audit query 13/13, context packs 10/10, sessions 22/22, OpenAPI 5/5, console 165/165 and forced-RLS trace guards; demo: `demos/cpr-20-context-planning.sh`. Current immutable Knowledge now feeds the session planner directly; bounded candidates, exact selections, reason/score components, budgets, versions, degradation, rendered hash and revision-specific feedback persist under four trace-retention modes. Every item and source is decided before persistence and again before disclosure; denied material leaks no object-shaped trace, and authored pack/skill blocks are conservatively masked when exact historical authority cannot be reconstructed. Five generated operations grow the contract to 67: run list/detail, feedback, ordinary session-scoped Knowledge query and a separately diagnostics-gated evaluation lens. CLI, MCP, evaluation and audit use immutable Knowledge addresses through public APIs. The record composer, `index_tier` suite and temporary recall refusal tombstones are deleted, migration 0051 translates no old rows, and the isolated demo reports 55 Knowledge items, 47 plans, 75 selections, 2 feedback rows and zero old records. `make ci` and full `make db-test` pass.
- [x] [CPR-21: Context Inspector and outcome feedback](CPR-21.md) — done 2026-08-24, no new ADR or migration, AC tests: pure context/feedback rules 7/7, real-component retention/refusal acceptance 6/6, complete console 179/179, gateway context 3/3 and sessions 22/22. `/console/context-runs/{id}` renders CPR-20's generated freshly re-authorised trace: retained task/block, selected immutable Knowledge and provenance, current/stale/superseded state, reasons/scores/rank/tokens, budget, versions, degradation, hash and visible exclusions. The five explicit feedback acts bind one exact run selection and revision; selection alone records no helpfulness. Full/redacted/hashes-only/disabled modes stay distinct, denied detail remains aggregate-only, and hashes-only grows no frontend address or feedback target. Session timeline entries now link the inspector and say `Synveda supplied N knowledge items`; the server freshly decides each addressed selection before counting so a shared run leaks none of another principal's private revision. OpenAPI remains 67 operations and schema epoch 2 remains 49 migrations. Production build, `make ci` and full `make db-test` pass; interactive browser QA was unavailable because no browser was connected, so no visual-run claim is made.
- [x] [CPR-22: Core individual and small-team MVP acceptance](CPR-22.md) — done 2026-08-24, no new ADR, migration, API or policy/audit vocabulary. The consolidated PulseBoard public-API/database AC is 1/1 and the complete capture suite 4/4: Alice's 4 session events become candidates only; she publishes 2 project items and 1 principal-private preference through 3 applied VedaFlow changes and dismisses 1. A clean Bob session receives both project revisions and their exact event sources but no private id/content/query result. His captured correction replaces the exact `X-Request-Id` revision through a fourth change, retains an explicit `supersedes` edge, and a third clean run selects `traceparent` while the inspector explains the obsolete item only as excluded history. Database state is 3 sessions, 5 events/candidates/decisions, 4 Knowledge items/revisions/changes, 2 runs, 3 selections and zero records; the audit chain verifies with PDP and semantic action evidence but no content, and all three deleted global routes stay 404. Demo: `demos/cpr-22-mvp-acceptance.sh`; context 3/3, console 179/179, `make ci` and full `make db-test` pass. This records the explicit core-MVP checkpoint; CPR-14 remains the genuine live Claude evidence rather than this deterministic run being relabelled live.
- [x] [CPR-23: Immutable skill versions, bindings and usage](CPR-23.md) — done 2026-08-24 from `9b8ad04`, ADR-0085, migration `0052_versioned_skills`. Stable Skill aggregates now own immutable content-addressed versions; revisioned project/principal bindings follow current, pin, disable, re-enable and roll back without rewriting history. Every install/update/binding effect is a typed VedaFlow change with live PDP, precondition, scan/rubric and content-free audit checks; availability and context use the same exact binding/version resolution. Eight usage stages distinguish host-observed from model-reported evidence, and the controlled validation harness executes no bundle code. The old mutable drafts/files, `skill/published` channel distribution, special checklist/override mutations and duplicate telemetry are deleted without translation. Official Agent Skills is pinned to upstream `69ef37e`; the generated contract grows 67 → 85 operations. AC: gateway 1/1, RLS/immutability 1/1, RLS completeness 1/1, OpenAPI 5/5, policy packs 7/7, CLI 157/157, console 179/179; `demos/cpr-23-versioned-skills.sh`, `make ci` and full disposable-Postgres `make db-test` pass.
- [x] [CPR-24: Skills Library product experience](CPR-24.md) — done 2026-08-24 from `89b5f79`, no new ADR or migration. The generated-contract Skills Library now has a policy-aware catalogue, stable detail routes, exact immutable version/file/provenance/scan evidence, personal and project binding controls, complete-bundle install/update, explicitly non-executing validation-sandbox tests, controlled-client history and exact-version usage split into host-observed and model-reported evidence. Declared tools are visibly metadata, not permission; generated capability forecasts hide unavailable mutations while the gateway and exact availability resolver remain authoritative. The hand-written Skill request, mutable-Skill branch in console/CLI proposal review, ten-fixture corpus and dead styles/tests are deleted; artifact-neutral Advanced Reviews remains. AC: Skills 10/10, shared review 5/5, console 186/186, CLI 151/151, production build and `make ci` pass. `make db-test` is N/A; interactive Browser QA was unavailable because this environment exposed no browser instance, so real-component SSR plus the production build is the honest UI evidence.
- [x] [CPR-25: Trusted MCP server catalogue and project bindings](CPR-25.md) — done 2026-08-25 from `07ce9f3`, ADR-0086, migration `0053_tool_registry`. Stable ToolServer aggregates own immutable raw/normalised MCP evidence; changed source, transport, authentication, permission or capability shape creates a quarantined version, while unchanged discovery is idempotent. Typed VedaFlow changes govern approval and revisioned exact-version project bindings, so approving another version cannot silently move a project. Sixteen generated operations provide bounded manifest/client-config import, version/schema comparison, bindings, secret-reference-only configuration and immutable discovery/list test reports; the gateway never launches imported stdio commands or proxies execution, and descriptions grant nothing. The official stable stateless MCP `2026-07-28` contract is pinned to release commit `5f5440b`; HTTP+SSE and connection-session semantics are refused. All six tables are forced-RLS, each read is PDP-filtered, and audit is content/secret-free. AC: types 5/5, gateway boundary 3/3, public lifecycle 1/1, policy PASS, RLS completeness 1/1, OpenAPI 5/5, generated console contract 186/186; `demos/cpr-25-tool-registry.sh`, `make ci` and full `make db-test` pass. The deterministic discovery report is fixture evidence, not a live external-server claim.
- [x] [CPR-26: MCP Tools catalogue product experience](CPR-26.md) — done 2026-08-25 from `9845186`, no new ADR, migration, API, policy or audit vocabulary. The generated-contract Tools catalogue and stable detail address now expose immutable source/capability evidence, protocol/transport/authentication shape, metadata-validation and honest no-executable-scan state, tools/resources/prompts with schemas, approved-head comparison, obvious quarantine with common VedaFlow review linkage, exact revisioned project bindings, discovery-only trusted-adapter reports and secret-safe generated configuration. Capability forecasts hide unavailable import/discovery/test/binding controls while the gateway remains authoritative; quarantined versions never enter the binding picker, approval never moves an existing exact binding, declared metadata grants nothing and the UI repeats the credential-redaction boundary before rendering open JSON. The sole `Planned` Tools placeholder is deleted. AC: helpers/components 10/10, complete console 196/196, production build and `make ci` pass. `make db-test` is N/A; no in-app browser was exposed, so real-component SSR plus the production build is the honest UI evidence.
- [x] [CPR-27: OKF v0.2 knowledge exchange adapter](CPR-27.md) — done 2026-08-25 from `98f5bcd`, ADR-0087, migration `0054_okf_imports`. The leaf adapter pins canonical OKF v0.2 at `GoogleCloudPlatform/open-knowledge-format@ad30107`, intentionally rejects v0.1 fallback, and validates bounded directory/zip/tar/checked-out-tree bytes without network access or execution. Immutable import jobs, artifacts and mappings dry-run additions, updates, duplicates and conflicts before idempotent materialisation creates capture candidates only; later acceptance remains the existing PDP/VedaFlow Knowledge path and normalises artifact plus declared-source provenance. Deterministic export re-authorises each current Knowledge item and preserves stable paths, relations, verification, staleness and unknown extensions. Four new tenant tables are forced-RLS; audit is content-free; traversal, links, special/binary/executable input, credential-shaped remotes and expansion bombs are refused. OpenAPI grows 101 → 106 operations and the generated New Learnings surface distinguishes OKF artifacts from session evidence. AC: adapter 6/6, types 1/1, store 1/1, public API 1/1, capture 4/4, OpenAPI 5/5, RLS 1/1, console 197/197; isolated `demos/cpr-27-okf-v02.sh`, `make ci` and full `make db-test` (`synveda_test_1177`) pass. No remote Git/source verification is claimed.
- [x] [CPR-28: OKF import and export product workflows](CPR-28.md) — done 2026-08-25 from `0dbf163`, no new ADR, migration, API, policy or audit vocabulary. `synveda okf validate|inspect` runs the pinned v0.2 adapter locally over a selected directory/archive; import packages inert bytes for the public immutable plan and optionally its separate idempotent candidate materialisation; export revalidates the returned pin, canonical paths, hashes and digest before atomically publishing a new private directory. The generated-contract project **Import / Export** page exposes source/revision, validation, immutable history, additions/updates/duplicates/conflicts, exact unknown metadata, candidates/New Learnings, current Knowledge selection and deterministic output. Neither client runs Git/content, grants server-path authority, publishes Knowledge directly, schedules synchronisation or invents another bundle format. AC: adapter 6/6, CLI 150/150, console 207/207, production build and public OKF API 1/1; `demos/cpr-28-okf-workflows.sh`, 77-script demo-drift gate and `make ci` pass. `make db-test` is N/A because persisted and governed behavior is unchanged.
- [x] [CPR-29: Public contract and client convergence](CPR-29.md) — done 2026-08-25 from `683a17d`, ADR-0088, no migration, Cedar or audit vocabulary change. One executable catalogue constructs all **156** authenticated `/v1` operations and the OpenAPI suite proves exact method/path parity in both directions; the generated document has 238 schemas. The console transport retains only HTTP/session mechanics and every application operation/type is generated. Ordinary service/audit CLI commands use the public gateway; generic MCP reads session, scoped Knowledge, available Skill and approved project Tool metadata through public routes, copies no executable configuration or secret reference, and treats declared tools as non-authorising evidence. A source guard pins the Claude adapter to documented public session operations with no store or retired global-route authority. Direct local authority remains only for documented bootstrap/reset/migration, key/secret custody and break-glass policy-pack operations. AC: OpenAPI 6/6, service identities 5/5, audit query 13/13, CLI service 1/1, audit 2/2, MCP 44/44, complete CLI 156/156 + MCP corpus 5/5, console 208/208, Claude adapter 98/98, generated API and 78-script demo drift gates; isolated `demos/cpr-29-public-contract.sh`, `make ci` and full `make db-test` pass.
- [x] [CPR-30: Governed runtime configuration artifacts](CPR-30.md) — done 2026-08-25 from `b33ba51`, ADR-0089, migration `0055_governed_configuration`. Stable aggregates own immutable content-hashed Configuration versions; revisioned nearest-scope bindings follow current or pin an exact version. Canonical personal/team/enterprise documents drive policy-pack selection, capture, context budget/channels/trace retention, type-aware freshness, Skill/Tool advertisement and provider allowlists without edition branches. Every create/publish/bind/update/rollback is a typed VedaFlow change under live PDP and content-free audit; capture and context retain exact version/digest evidence, including a separately authorised and visibly unreviewed candidate channel. Four tables are forced-RLS, all object reads decide independently, and the mutable default/assignment tables and routes are deleted without translation. The generated contract grows 156 → 162 operations and 238 → 255 schemas; generated console and public-HTTP CLI clients expose templates, versions, comparison, effective resolution and rollback. AC: domain 4/4, public API 1/1, capture 4/4, context 3/3, approvals 6/6, packs 7/7, PDP 11/11, RLS 83/83, OpenAPI 6/6, console 210/210; isolated `demos/cpr-30-governed-configuration.sh`, `make ci` and full `make db-test` pass.
- [x] [CPR-31: Governed auto-apply and policy relaxations](CPR-31.md) — done 2026-08-25 from `ed7d233`, ADR-0090, migration `0056_governed_policy_relaxations`. The mutable AUTHZ-4 row/effect/config plane is deleted without translation. Stable relaxation aggregates own immutable content-hashed versions naming one provisioned subject, exact non-personal scope, closed `knowledge.read` action, sensitivity ceiling, requested window, database-time hard expiry, creator, exact approvers and frozen effective Configuration evidence. Create, revise and revoke are typed `Policy/apply` VedaFlow changes behind ownership, `relaxation.write`, proposal, live matrix, payload and stale-head checks; open collaboration auto-applies the same persisted change while standard retains it for review or terminal rejection. Cedar alone consumes exact active matches, and current Configuration may narrow them; private scopes, quarantine and service confinement remain overriding forbids. Three new tenant tables are forced-RLS; the public cursor API, generated console and HTTP-only CLI cover the lifecycle; expiry restores denial without a worker and the sweep adds content-free chain evidence once. AC: types 210/210 + serde 50/50, policy including relaxation 3/3, gateway 2/2, RLS/immutability/expiry 2/2, OpenAPI 6/6, audit 27/27, CLI 155/155 + MCP corpus 5/5, console 209/209 + production build, retrieval 53/53; isolated `demos/cpr-31-governed-relaxations.sh`, `make ci` and full `make db-test` pass.
- [x] [CPR-32: Unified approvals across governed artifacts](CPR-32.md) — done 2026-08-25 from `92819516ee35abf3f5a0fe6cd8c0658f666269af`, ADR-0091, migration `0057_unified_artifact_approvals`. The existing forced-RLS VedaFlow proposal now carries mandatory immutable typed references for Knowledge (including OKF evidence), Skills, Tool servers/bindings, Configuration, policy relaxations and authored assets; every caller binds the stable id, exact operation/version and stale-head precondition without a parallel review table. Live approval matrices merge author self-review and distinct effect-actor restrictions monotonically with roles, subjects and distinct-person counts. Approve/reject name the exact commit; apply/publish repeats PDP, matrix, separation and artifact-head checks. The generated 164-operation contract and Advanced Reviews expose family filtering, typed evidence, content-free timelines, exact verdicts, proposer cancellation via withdrawal and effect execution; New Learnings and session-event quarantine remain distinct state boundaries. AC: types 212/212 + serde 50/50, policy 77/77, VedaFlow 73/73 + object store 10/10, gateway family suites 27/27, OpenAPI 6/6, console 210/210, policy-pack store 5/5, forced RLS 83/83; isolated `demos/cpr-32-unified-approvals.sh` proves 81 typed proposals, seven families, 23 commit-bound reviews, regulated three-person separation and no audited content; `make ci`, the 80-script demo gate and full `make db-test` (`synveda_test_43866`) pass.
- [x] [CPR-33: Context-platform audit query and deterministic export](CPR-33.md) — done 2026-08-25 from `cf52f34b4d408ef147310041f9367b1e445b4162`, ADR-0092, migration `0058_context_audit_export`. The existing tenant-complete `AuditRead` chain now answers exact typed artifact family/id/version, session and context-run filters; every terminal Knowledge, Skill, Tool, Configuration and relaxation act repeats the immutable artifact reference rather than requiring inference from display text. Context composition records exact selected Knowledge revisions, advertised Skill binding/versions, effective Configuration aggregate/binding/scope/version/hash/policy and gathered relaxation versions. The Knowledge question separates valid time from as-known transaction time, walks backwards by sequence and resolves only content-free immutable revision evidence; erased or hashes-only delivery remains visibly unresolved without an invented address. `GET /v1/audit/export` freezes the head before its own audited read, pages one contiguous canonical prefix and supplies tenant genesis/head hashes for offline verification; the public-HTTP CLI assembles, verifies and atomically writes without overwrite, while the generated console downloads the same frozen walk. No replay, second audit store, content resolution, SIEM stream or WORM claim was added. The contract grows 164 → 165 operations and 262 → 264 schemas; epoch 2 has 56 migration files, 700 SQLx descriptions and 91 forced-RLS tenant tables. AC: audit 23/23 + tamper 7/7, gateway audit 16/16, five terminal-reference family regressions, CLI audit 4/4 + complete CLI 157/157, OpenAPI 6/6, console 212/212 and RLS completeness PASS; isolated `demos/cpr-33-audit-export.sh` proves seven self-audited export reads, 49 typed artifact events and one tenant-leading payload index; `make ci`, the 81-script demo gate and full `make db-test` (`synveda_test_51591`) pass.
- [x] [CPR-34: Directory adapter convergence](CPR-34.md) — done 2026-08-25 from `3c61e5e0fa35f8e9a0056f1e7d53a19bfe43debc`, ADR-0093, migration `0059_directory_adapter_convergence`. SCIM push and scheduled Entra/Okta pull now converge on source-qualified directory users, shared Groups and identity-keyed `group_members`; stable provider ids survive display-name changes, and incomplete passes infer no absence. Directory assignments are separately `MembershipGrant`-decided, idempotent group-subject `scope_grants` with source evidence and chained create/revoke events; direct APIs refuse source-owned rows. Disable, membership removal and Group retirement withdraw authority through the same RLS/Cedar anchor path. The second SCIM group graph is deleted without translation and affected pre-cut databases are refused with the reset instruction. The contract grows 165 → 167 operations and 264 → 266 schemas; epoch 2 has 57 migrations, 694 SQLx descriptions and 89 forced-RLS tenant tables. AC: connectors 5/5; store access 30/30, anchors 13/13 and sync 8/8; gateway access 18/18, sync 9/9, SCIM 10/10 and anchors 9/9; OpenAPI 6/6, console 212/212 and RLS 83/83; `demos/cpr-34-directory-convergence.sh`, the 82-script demo gate, `make ci` and full fresh-database `make db-test` pass. Entra/Okta evidence remains captured/transcribed, not live.
- [x] [CPR-35: Context-platform key and secret convergence](CPR-35.md) — done 2026-08-25 from `13ba0596c75f46f30d77e605c4f7548ae44af425`, ADR-0094, migration `0060_context_secret_plane`. Stable UUIDv7, scope-bound secret references now survive logical value and tenant-DEK rotation while revoked/corrupt/wrong-kind/cross-tenant Tool and directory references fail closed; durable re-encryption retains identity and logical revision, and the new sealed export contains complete Knowledge history/provenance/relations plus audit with no Record section or old archive reader. Both replacement tables are forced-RLS. AC: types 214/214, crypto 39/39, store keys 14/14 plus Knowledge export and RLS completeness, gateway Tools 2/2 and directory sync 10/10, CLI 158/158, OpenAPI 6/6 and console 212/212; `demos/cpr-35-key-secret-convergence.sh`, the re-cut TEN-4 demo, 83-script drift gate, `make ci` and full fresh-database `make db-test` (`synveda_test_69956`) pass. No unsupported external key-provider claim was added.
- [x] [CPR-36: One-runtime deployment convergence](CPR-36.md) — done 2026-08-25 from `2e70aaf5a10a74dea6f224e1fefff4e81d798db3`, ADR-0095, no migration or public API change. Host/release Compose and Helm now run the same gateway/schema/contract with Configuration as governed data rather than an edition switch. Local init converges a separate `synveda_gateway` LOGIN that is non-superuser/non-BYPASSRLS and inherits `synveda_app`; Helm verifies the same properties, while migration and tenant admission retain the admin identity. `init --demo`, bundled identities and the dead release seeder are deleted without aliases. The deployment gate renders two Compose shapes and Helm, checks current OpenAPI/CLI and least-privilege DSNs, and packages twice to prove stale replacement. AC: focused CLI **19/19**, deployment fixtures **4/4**, epoch **10/10**, OpenAPI **6/6**, Helm lint/render, `demos/cpr-36-deployment-convergence.sh`, the 84-script demo gate and `make ci` pass; full fresh-database `make db-test` passes on `synveda_test_77612`, removed by the harness. The chart remains one gateway replica with restart-shaped upgrades; binaries remain unsigned and no Windows/HA/old-schema-translation claim was added.
- [x] [CPR-37: Conflict, supersession and freshness engine](CPR-37.md) — done 2026-08-25 from `ca3730f7d43b0bd6fd2a1a22d86d48093eea7395`, ADR-0096, migration `0061_conflict_freshness.sql`. Bounded post-PDP classification at direct write and capture time now records exact revision/candidate evidence in forced-RLS conflict sets; ambiguous writes are transitional and absent from ordinary search and context until revision-aware VedaFlow resolution. Resolution supports support/duplicate evidence, separate truth, governed supersession, future-valid transition and archive without rewriting history. Freshness evaluates the exact effective Configuration with explicit dates and type/source/use/repository signals, while cursor queries compose current, valid-time, transaction-time, history and transitional semantics. The generated Knowledge Browser adds policy evidence, staleness, conflict comparison and resolution; denied members suppress the whole conflict set and guessed resolution cannot become an existence oracle. AC: Knowledge types 7/7, lifecycle 5/5, capture 4/4, ingest 2/2, context 3/3, OKF 2/2, RLS 83/83, OpenAPI 6/6 and console 215/215; `demos/cpr-37-conflict-freshness.sh`, the 85-script demo gate, `make ci` and full fresh-database `make db-test` on removed `synveda_test_85309` pass. Epoch 2 has 59 migration files, 92 forced-RLS tenant tables, 726 SQLx descriptions and a 171-operation/272-schema contract.
- [x] [CPR-38: Bounded graph-augmented retrieval](CPR-38.md) — done 2026-08-25 from `7951d77`, ADR-0097, migration `0062_bounded_graph_retrieval`. ContextRun v2 expands only already-authorised lexical/semantic anchors across the immutable Knowledge-relation graph, with governed 2-hop, fan-out, candidate, time and graph-token bounds and a fresh PDP decision at every frontier, endpoint, main-transaction revalidation and trace render. Six supporting relation types rank; contradictions remain zero-weight warnings. Candidate rows keep anchor/edge/hop/freshness/current/final components and forced-RLS immutable trace steps retain full/redacted addresses or hashes-only evidence. Every failure/bound preserves the lexical/vector result with an exact degradation; stale, superseded and transitional nodes cannot become current. The Record graph/linker/runtime and obsolete GRPH-1/GRPH-4 demos are deleted without translation, closing GRPH-3 by subsumption. AC: types focused 1/1; Knowledge 6/6; Configuration 1/1; ContextRun 4/4; RLS 79/79; OpenAPI 6/6; console 215/215; isolated `demos/cpr-38-bounded-graph.sh`, the 84-script demo gate, `make ci` and full fresh-database `make db-test` (`synveda_test_93371`, removed) pass. Epoch 2 has 60 migration files, 90 forced-RLS tenant tables, 704 SQLx descriptions and a 171-operation/274-schema contract.
- [ ] [CPR-39: Adapter conformance and second verified client](CPR-39.md) — implementation complete from `12e393a` under ADR-0098; external live criterion pending. `adapters/registry.json` is now the single authority for five evidence levels, exact criteria, tested versions, lifecycle contracts, limitations, content-addressed authentic fixtures and MCP config projection. Generated console onboarding and `docs/CLIENT_SUPPORT.md` cannot drift from it; CI refuses a config-only `verified` claim, a missing criterion, forged captured evidence or changed bytes. Claude Code 2.1.241 remains verified from CPR-14; Claude Desktop 1.25927.0 and Zed 1.13.2 remain captured. Cursor's current local Hooks v1 contract is sufficient in shape but no Cursor executable/authenticated session or authentic frame exists here; installed VS Code 1.133.0 has no authenticated profile and its Preview contract lacks SessionEnd. No replay is called live. AC evidence available here: registry gate 4/4, CLI config 28/28, MCP corpus 5/5, Claude adapter 98/98, console 216/216 plus production build, isolated `demos/cpr-39-adapter-conformance.sh`, the 85-script drift gate and `make ci` pass. No database behaviour changed, so `make db-test` is N/A. The resulting commit is recorded by CPR-40.
- [x] [CPR-40: Context-platform product and trust evaluation](CPR-40.md) — done 2026-08-26 from `0958d6936c4bbaf6b14a3ab69bc0d5cd48eec5a9`, ADR-0099. Eighteen exact product cases retain eight distinct outcome signals and six zero-count trust gates in revision/runtime-labelled reports. The evaluator now uses current sessions, governed capture/VedaFlow, immutable Knowledge and separately authorised query lenses over real principal/project/workspace/tenant scopes; no Record translator or ContextRun enumeration remains. Safe JSON entries plus exact Knowledge/revision/source addresses close structural and attribution forgery; bounded lexical input survives the 4,927-turn corpus. Ruleset `builtin@4` measures 0.983/0.914 extraction precision/recall. Final gates: default 6/6 plus 50 fixtures, all QA tiers and 1,276 probes; product 18/18 with all six trust counts zero; 10k security 10,876 probes, 9/9 controls and zero leaks; BGE-M3 10/10 at 0.800 bounded precision; authentic Stage-H all 4,927 turns and ten instances, zero empty/unattributed blocks, 0.643 retrieval recall and explicit 1,800-second sparse-index misses. Focused suites, `make eval-check`, `make ci` and full fresh-scratch `make db-test` pass. Model-backed extraction remains honestly unmeasured without a credential/vLLM endpoint. The resulting commit is recorded by CPR-41.
- [x] [CPR-41: One-command realistic product demo](CPR-41.md) — done 2026-08-26 from `036eab7`, ADR-0100. `synveda demo start --profile personal|team|governed`, `status` and force-explicit archival reset package the PulseBoard tour as a resumable authenticated public-API client: one runtime Configuration, workspace/project/repository, closed sessions, reviewable capture, private/project Knowledge, clean reuse, explicit supersession, trace, governed Skill/MCP and candidate-only OKF. A real Bob profile proves teammate reuse when present; otherwise the command returns a one-time invite, stores no token and makes no teammate claim. The fresh-tenant cycle is closed only for an administrator's first exact canonical Configuration and matching first binding; both remain typed VedaFlow changes with PDP/effect re-authorisation and audit, and a tenant-root transaction lock gives concurrent requests exactly one winner. Fixed synthetic state is atomically mode-0600, direct store/product SQL and legacy routes are absent, and deterministic hashing is labelled lexical-only. Final focused/demo/CI/database evidence is recorded in the implementation queue; the resulting commit is recorded by CPR-42.
- [x] [CPR-42: Context-platform security and product-integrity audit](CPR-42.md) — done 2026-08-26 from `8a4b944`, ADR-0078 amended. Four confirmed Claude adapter defects are closed: every automatic/manual read verifies payload hashes; unreadable, malformed and future-version state is held rather than overwritten; a spool cannot cross gateway origins; and diagnostic exception/payload/secret bytes cannot reach the local log. `make check-context-security` adds 5/5 mutation tests and pins 27 cross-layer adversarial boundaries plus non-executing Skill/MCP/OKF handlers, exact read-only MCP methods, public-client storage independence and runtime spool guards. AC: adapter 103/103, CLI 2/2, OKF 5/5, Skill 12/12, isolated `demos/cpr-42-security-integrity.sh`, 88-script drift gate, `make ci` and fresh-scratch `make db-test` (`synveda_test_49069`, removed) PASS; product 18/18 with all six trust counts zero; full security 10,000 variants/10,876 probes, 9/9 controls and zero tenant/scope/sensitivity/unattributed/watermark-gap findings. `docs/SECURITY.md` records the threat boundary and residual limits; no replay is called live. The resulting commit is recorded by CPR-43.
- [x] [CPR-43: Final context-platform hard cut](CPR-43.md) — done 2026-08-26 from `9bda16bc5bf471b6de2359a1396522cee42cc62d`, feature commit `2f468cdc73738450d1e29abd4d16bd2bcde81938`, ADR-0069 amended. The 60-file development chain is replaced by pure-DDL `0001_context_platform.sql`: epoch 3, 87 tables, two current views, 83 tenant tables with enabled/forced RLS, and no data mover. Record storage/search/promotion/retention, Tantivy, PGMQ, AGE and obsolete graph/runtime/client residue are deleted; Knowledge, capture, ContextRun and governed artifacts are the only current paths. OpenAPI/router/generated client agree at 171 operations / 272 schemas; 605 SQLx descriptions are current. The 375-file hard-cut gate and 3/3 mutation tests classify or reject every Prompt-33 term. AC: epoch 10/10, forced-RLS inventory 1/1, OpenAPI 6/6, CLI hard-cut 4/4, console 216/216, Claude adapter 102/102, CPR-2 and CPR-43 demos, full `make ci` and fresh `make db-test` (`synveda_test_74163`, removed) PASS. Product 18/18 and all six trust counts pass; default, BGE-M3 and 10k-security gates pass; the authentic 4,927-turn Stage-H rerun measures all ten instances at 0.643 recall with zero empty/unattributed blocks and retains explicit sparse-readiness misses. Deterministic Claude lifecycle passes 1/1. The existing genuine Claude 2.1.241 live run remains separately labelled; its closeout rerun was refused by execution approval, no extraction-model credential exists, and CPR-39's real second-client criterion remains externally blocked.
- [ ] [CPR-44: Production hardening and maintainability cut](CPR-44.md) — in progress on `refactor/production-hardening-nasa` from fetched source head `37fd12b1aa0504d18f02cd72ce7b284f672ef12f`; ADR-0101.
- [ ] Prompts 13–33 — filed by the prompts that run them (see §9 of the implementation record for the ordering this baseline commits to, and §10 for where a prompt's own text diverged from it).
