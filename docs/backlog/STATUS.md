...
- [ ] [CPR-13: The demo corpus re-point](CPR-13.md) — filed 2026-08-23 by CPR-12. **45 of the 67 shell scripts under `demos/` were dead**, and had been since CPR-7 (2026-08-20), which deleted `synveda role bind`, `synveda hierarchy` and `/v1/hierarchy/*` whole — correctly, per ADR-0074 — re-pointed the code, the tests, the CLI and the docs, and did not re-point the demos. **Nothing said so.** CPR-7, CPR-8, CPR-9, CPR-10 and CPR-11 all record clean runs, because no gate runs a demo: `make ci` builds, lints, tests and checks the backlog, the ADR statuses, the API types, the benchmarks, the chart images and two licence rules, and does not open a single `.sh` file under `demos/`. CLAUDE.md's definition of done says a feature is done "ONLY when its acceptance criteria pass, demonstrated by a test or a runnable demo script under `demos/`" — and for most of Phases 1–3 the demo half of that sentence currently names a script that exits non-zero on its fourth line. **Not CPR-12's to fix**: 28 of its 32 observe/inject/recall demos are inside the dead 45, so re-pointing their call sites produces scripts still dead one command earlier, and re-pointing the placement half means rewriting each script's setup narrative on the governed scope model — a prompt, not a side effect. CPR-12 fixed the four that were live and deleted `ctx-5-recall.sh`, whose subject no longer exists. **The half that matters more is the gate**: `make check-demos`, failing when a demo names a CLI subcommand `synveda --help` does not list or a `/v1` path absent from the OpenAPI document. `check-backlog`, `check-adr-status`, `check-api-types` and `check-benchmarks` are four precedents and each exists because a document drifted from the tree once.
