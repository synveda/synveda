...
- [ ] [CPR-14: Live Claude Code session acceptance gate](CPR-14.md) — filed 2026-08-23 after CPR-12. **Implemented at replay/live-gateway level; live-client verification pending.** The deterministic gate runs genuine, versioned Claude Code frames through the built hook child process, public session API, epoch-2 Postgres, PDP, ingestion worker, timeline and verifying audit chain. It proves a SessionStart open plus context run, user/tool/assistant events, normal SessionEnd, private durable spool state, an outage with two pending entries, lost-ack overlap returning `duplicate` at the original sequence and `appended` at the next, exactly-once persistence, and message-free timeline/audit evidence. Fixture manifest + schema bind every byte to capture provenance, sanitisation and SHA-256; `make claude-acceptance` is the CI replay target. `make claude-acceptance-live` packages and installs through the real marketplace path, verifies Claude Code's own plugin/hook/MCP report and invokes real `claude -p`, but the 2026-08-23 run stopped at its honest prerequisite: Claude Code **2.1.241** was installed and unauthenticated (`loggedIn: false`, no isolated credential). No real client session ran, so this feature is not delivered and support claims retain that limitation. ADR-0079.
