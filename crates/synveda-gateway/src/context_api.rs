...
use synveda_types::knowledge::{
    KnowledgeLifecycleState, KnowledgeRelationType, KnowledgeRevision, KnowledgeSource,
    KnowledgeType,
};
...
async fn collect_planned_candidates(
    ...
    for (item_id, seed, lifecycle_exclusion) in pools {
        ...
        } else if graph::stale_at(tx, tenant_id, &snapshot, at).await? {
...
async fn plan_context_run(
    ...
    let (graph_expansion, graph_attempted) = graph::expand_graph_with_fallback(
        state,
        &mut tx,
        tenant_id,
        &candidates,
    ...
    .await?;
    let graph_degradation = graph_expansion.degradations().to_vec();
    policy_exclusion |= graph::merge_graph_expansion(
        state,
        &mut tx,
        tenant_id,
        &mut candidates,
