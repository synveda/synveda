...
//!
//! Re-anchored on the session ledger by CPR-12 (ADR-0078 decision 1): the scan
//! seam moved from `/v1/observe` to `POST /v1/sessions/{id}/events` and the
//! review row from `observe_quarantine` to `session_event_quarantine`. The
//! criteria did not change, and neither did this suite's shape.
//!
...
fn batch(_session: &str, events: Vec<Value>) -> Value {
    json!({ "events": events })
}
...
async fn storage_contains(pool: &PgPool, tenant: TenantId, seed: &str) -> bool {
    ...
    sqlx::query_scalar!(
        ...
        select exists (
            ...
            union all
            select 1 from session_event_quarantine
            where tenant_id = $1
...
async fn seeded_secrets_never_reach_storage_in_any_mode() {
    ...
    let run = session_seed::seed_run_for(&pool, tenant, "mem2", "alice")
        .await
        .session_id;
    ...
    assert_eq!(body["events"][0]["outcome"], "quarantined", "{body}");
    assert_eq!(body["events"][1]["outcome"], "appended", "{body}");
    assert_eq!(body["events"][2]["outcome"], "appended", "{body}");
    assert!(
    ...
    let quarantined_id = body["events"][0]["event"]["id"]
        .as_str()
    ...
    .await;
    assert_eq!(status, StatusCode::OK, "{body}");
    assert_eq!(body["appended"], 1, "{body}");
    assert_eq!(
        body["quarantined"], 0,
    ...
    assert_eq!(body["appended"], 1, "{body}");
    assert_eq!(body["events"][0]["outcome"], "denied", "{body}");
    assert!(
    ...
    let (status, retry) = send(
        &app,
        request(Method::POST, &events_uri(run), &alice, Some(deny_batch)),
    )
    .await;
    assert_eq!(status, StatusCode::OK, "{retry}");
    assert_eq!(retry["events"][0]["outcome"], "denied", "{retry}");
    assert_eq!(retry["events"][1]["outcome"], "duplicate", "{retry}");
    ...
    let events = synveda_audit::tail(&mut tx, tenant, 100)
        .await
        .expect("read chain");
    // One chained event per batch, under the append's own action since
    // CPR-12: `memory.observed` named a plane that no longer exists.
    ...
    let quarantine_batch = observed
        ...
        .expect("the quarantining batch's event");
    assert_eq!(
        quarantine_batch.payload["redaction_rules"]["aws-access-key-id"], 1,
        "the rule summary rides the chain: {}",
        quarantine_batch.payload
    );
    let deny_batch_event = observed
        .iter()
        .find(|event| event.payload["denied"] == 1)
        .expect("the denying batch's event");
...
async fn quarantine_review_contract_holds() {
    ...
    let run = session_seed::seed_run_for(&pool, tenant, "mem2-review", "alice")
        .await
        .session_id;
    let alice = idp.user_token("alice");
    ...
    assert_eq!(status, StatusCode::OK, "{body}");
    assert_eq!(body["quarantined"], 1, "{body}");
    let event_id = body["events"][0]["event"]["id"]
        .as_str()
        .expect("event id")
        .to_owned();
    assert_eq!(
    ...
    let ghost = synveda_types::SessionEventId::new();
