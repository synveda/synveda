...
async fn inject_once(
    ...
    let body = json!({ "query": TASKS[session % TASKS.len()] });
    ...
    assert!(
        ...
        "every load composition must succeed, got {status}"
    );
    elapsed
}
...
async fn inject_median_under_budget_at_1k_concurrent_sessions() {
    ...
    let mut runs: Vec<synveda_types::SessionId> = Vec::with_capacity(USERS);
    for user in 0..USERS {
        ...
        // A run per reader: a composition names the session it is for since
        // CPR-12 (ADR-0078 decision 5).
        runs.push(
            session_seed::seed_run_for(&pool, tenant, &format!("ctx3-load-{user}"), &subject)
                .await
                .session_id,
        );
    ...
    for (user, token) in tokens.iter().enumerate() {
        inject_once(&app, token, runs[user], user).await;
    }
    ...
    for session in 0..SESSIONS {
        ...
        sessions.push(tokio::spawn(async move {
            ...
            inject_once(&app, token, runs[session % runs.len()], session).await
        }));
    }
    ...
    for _ in 0..PROBE_CONCURRENCY {
        ...
        workers.push(tokio::spawn(async move {
            ...
            loop {
                ...
                let token = &tokens[session % tokens.len()];
                latencies.push(
                    inject_once(&app, token, runs[session % runs.len()], SESSIONS + session).await,
                );
            }
        }));
    }
