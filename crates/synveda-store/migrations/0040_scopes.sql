-- CPR-3: the generic governed scope substrate (ADR-0068 decision 4, ADR-0070).
--
...
-- ── What this is not ─────────────────────────────────────────────────────
--
-- It is not `hierarchy_nodes` renamed. That table encodes a five-value rank
-- vocabulary (org, division, department, team, user) in a CHECK, a
-- root-must-be-an-org CHECK, and a store-side rule that a child must strictly
-- outrank its parent — so an individual is required to declare an
-- organisation containing a team before this product will hold a record.
-- `kind` here is a **shape**, not a rank: it decides only which kinds may be
-- a scope's parent, `org_unit` nests inside itself to arbitrary depth, and
-- nothing anywhere compares two kinds for order.
--
-- The old hierarchy is untouched by this migration and is deleted whole by
-- Prompt 6 of the context-platform programme. Nothing here reads it, writes
-- it, or is derived from it: no row of `hierarchy_nodes` becomes a row of
-- `scopes` (ADR-0068 decision 3 — there is no translator, by decision).
--
...
create table scopes (
    ...
    -- foreign key: a tenant root minted at admission has no author, and the
    -- identity model is re-cut by the next prompt of this programme — a
    -- reference to the table that model replaces would be a dependency on
    -- something already scheduled for deletion.
    created_by       uuid,
    ...
    constraint scopes_placement_check
        check (
            case kind
                ...
                when 'principal' then parent_kind = 'tenant'
                else false
            end
        ),
...
-- rewritten is not stable. What remains updatable is exactly what the two
...
-- in this substrate — a scope is what audit events, versions and bindings
