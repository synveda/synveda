...
if [ "$purge" = yes ]; then
  ...
  say "    $HOME_DIR/data/kms.key"
  say "They are legacy evaluation state. The CPR-45 cutover has no accepted"
  say "reinstall, bootstrap or reset path that claims to reuse them. This"
  say "script removed product files but deliberately preserved that state."
