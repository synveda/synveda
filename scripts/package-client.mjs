...
execFileSync("sh", [join(root, "scripts/release-version.sh"), version]);
...
try {
  mkdirSync(client);
  run("bash", [join(root, "scripts/package-plugin.sh"), version, scratch]);
  ...
  const upstreamRoot = pin.archive.replace(/\.tar\.gz$/, "");
  run("tar", ["-xzf", resolve(runtimeArchive), "-C", scratch, `${upstreamRoot}/bin/node`, `${upstreamRoot}/LICENSE`]);
  const runtime = join(client, "plugin/synveda/runtime");
  mkdirSync(runtime);
  copyFileSync(join(scratch, upstreamRoot, "bin/node"), join(runtime, "node"));
  copyFileSync(join(scratch, upstreamRoot, "LICENSE"), join(runtime, "LICENSE"));
  chmodSync(join(runtime, "node"), 0o755);
  if (run(join(runtime, "node"), ["--version"]).trim() !== `v${lock.version}`) throw new Error("private Node version mismatch");
  for (const relative of ["plugin/synveda/hooks/hooks.json", "plugin/synveda/.mcp.json"]) {
    ...
    function rewrite(value) {
      ...
      if (value.command === "node") value.command = "${CLAUDE_PLUGIN_ROOT}/runtime/node";
  ...
  copyFileSync(binary, join(client, "bin/synveda"));
  chmodSync(join(client, "bin/synveda"), 0o755);
  const cliVersion = run(join(client, "bin/synveda"), ["--version"]).trim();
  ...
  const archive = join(resolve(output), `synveda-client-${version}-${target}.tar.gz`);
  run("tar", ["-czf", archive, "-C", scratch, "client"]);
