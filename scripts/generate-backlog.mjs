...
//
// Running this OVERWRITES STATUS.md, which carries hand-written per-feature
// narrative that is not in PHASE_NOTES (STATUS.md says so at the top, and
// says to lift the paragraphs out of git first). Because writing happens at
// module scope, merely *importing* this file — `node -e "import(...)"`, a
// linter, an editor plugin — used to be enough to destroy them. It now
// refuses unless it is the program being run.
...
if (process.argv[1] !== fileURLToPath(import.meta.url)) {
  throw new Error(
    "generate-backlog.mjs rewrites docs/backlog/STATUS.md and must be run, " +
      "not imported: node scripts/generate-backlog.mjs",
  );
}
