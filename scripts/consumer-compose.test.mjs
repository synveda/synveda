...
test("the opt-in packaged candidate resolves fresh without any generated host secret files", (t) => {
  ...
  const publication = JSON.parse(readFileSync("docs/installation.json", "utf8"));
  if (publication.publication === "published") {
    assert.match(consumerGuide, /part of the matching published GitHub Release/);
  } else {
    assert.match(consumerGuide, /qualification artifact until the matching GitHub Release\s+is stable/);
  }
  assert.match(consumerGuide, /v0\.4\.0 launcher remains available/);
